...
func findNextEdge(node DBGNode, eID DBG_MAX_INT, rArr []sam.Record) (neID DBG_MAX_INT, recd sam.Record) {
	...
	var refIDArr []DBG_MAX_INT
	for _, v := range rArr {
		id, err := strconv.Atoi(v.Ref.Name())
		if err != nil {
			log.Fatalf("[findNextEdge] ref Name not numberical err: %v\n", err)
		}
		refIDArr = append(refIDArr, DBG_MAX_INT(id))
	}

	num := 0
	var teID DBG_MAX_INT
	for i := 0; i < bnt.BaseTypeNum; i++ {
...
func isInEdgesArr(arr []DBG_MAX_INT, e DBG_MAX_INT) bool {
	for _, v := range arr {
		if v == e {
			return true
		}
...
func paraFindShortMappingPath(rc chan [2]FastMapRecord, wc chan []DBG_MAX_INT, edgesArr []DBGEdge, nodesArr []DBGNode) {
	// totalM, totalI, totalD := 0, 0, 0
	for {
		rArr := <-rc
		if len(rArr[0].Minfo) == 0 {
			var guardPath []DBG_MAX_INT
			wc <- guardPath
			break
		}
...
func IsCyclePath(path []DBG_MAX_INT) bool {
	cycle := false
...
func IsNodeCyclePath(path []DBG_MAX_INT, edgesArr []DBGEdge) bool {
	// node array
	var narr []DBG_MAX_INT
...
func WriteShortPathToDBG(wc chan []DBG_MAX_INT, edgesArr []DBGEdge, numCPU int) {
	terminalNum := 0
	...
	for {
		...
		// write path to the DBG edgesArr
		{
			...
			// path = ReverseDBG_MAX_INTArr(path)
...
func IndexEID(arr []DBG_MAX_INT, eID DBG_MAX_INT) int {
	for i, id := range arr {
		if id == eID {
			return i
		}
...
func GetReverseDBG_MAX_INTArr(arr []DBG_MAX_INT) []DBG_MAX_INT {
	la := len(arr)
...
func FindConsistenceAndMergePath(a1, a2 []DBG_MAX_INT, eID1, eID2 DBG_MAX_INT) (mergeArr []DBG_MAX_INT, consisFlag bool) {
	idx1 := IndexEID(a1, eID1)
	idx2 := IndexEID(a1, eID2)
...
func SubstituteEdgeID(nodesArr []DBGNode, nID, srcEID, dstEID DBG_MAX_INT) bool {
	var success bool
	for i := 0; i < bnt.BaseTypeNum; i++ {
...
func GetDirection(node DBGNode, eID DBG_MAX_INT) (direction uint8) {
	if eID == 0 {
...
func connectNodeID(e1, e2 DBGEdge) DBG_MAX_INT {
	if e1.StartNID == e2.StartNID || e1.StartNID == e2.EndNID {
		return e1.StartNID
	} else if e1.EndNID == e2.StartNID || e1.EndNID == e2.EndNID {
		return e1.EndNID
...
func DeleteEdgeID(nodesArr []DBGNode, nID, eID DBG_MAX_INT) (success bool) {
	for i := 0; i < bnt.BaseTypeNum; i++ {
...
func CheckConsistenceArr(arr1, arr2 []DBG_MAX_INT, eID DBG_MAX_INT) (mp []DBG_MAX_INT, ok bool) {
...
func FoundAllPath(edge DBGEdge, eIDArr []DBG_MAX_INT) (path Path, num int) {
...
func GetUniqueOne(src, dst []DBG_MAX_INT) (unique DBG_MAX_INT) {
	for _, sID := range src {
		for _, dID := range dst {
			if sID == dID {
				unique = sID
...
/*func FindMaxUnqiuePath(edgesArr []DBGEdge, nodesArr []DBGNode) [][]DBG_MAX_INT {

consistenceA := make([][]DBG_MAX_INT, len(edgesArr))
...
func GetNeighbourEID(eID DBG_MAX_INT, nID DBG_MAX_INT, edgesArr []DBGEdge, nodesArr []DBGNode, max_insert_size int) (neighbourEIDArr []DBG_MAX_INT) {
...
func IsIntersection(rightEIDArr, leftEIDArr []DBG_MAX_INT) bool {
...
func IsContained(subArr, allArr []DBG_MAX_INT) bool {
...
func IsInPathMat(pathMat []Path, eID DBG_MAX_INT) bool {
	for _, p := range pathMat {
		if IsInDBG_MAX_INTArr(p.IDArr, eID) {
			return true
		}
...
func GetInterNodeID(edgeSrc, edgeDst DBGEdge) DBG_MAX_INT {
...
func GetMergePathArr(EpathMat []Path, edgesArr []DBGEdge, nodesArr []DBGNode) (mergePathArr []Path) {
	...
	for _, pi := range EpathMat {
		var p Path
		p.IDArr = make([]DBG_MAX_INT, len(pi.IDArr))
	...
	for i := 0; i < len(pathMat); i++ {
		...
		// check unique path
		var np Path
		np.IDArr = make([]DBG_MAX_INT, len(p.IDArr))
...
func FindMaxUnqiuePath(edgesArr []DBGEdge, nodesArr []DBGNode) [][2][]DBG_MAX_INT {

	consistenceA := make([][2][]DBG_MAX_INT, len(edgesArr))
...
func InsertIDArr(RefIDArr []DBG_MAX_INT, ID DBG_MAX_INT) []DBG_MAX_INT {
...
func GetLARecord(lafn string, RefIDMapArr []DBG_MAX_INT, lac chan []LA, numCPU int) {
	fp, err := os.Open(lafn)
	if err != nil {
		log.Fatalf("[GetSamRecord] open file: %s failed, err: %v\n", lafn, err)
	}
...
func GetLARegion(laArr []LA, eID DBG_MAX_INT) []LA {
	b, e := -1, -1
...
func findEdgeLA(eID DBG_MAX_INT, laArr []LA, gapRegArr []GapRegion, QuyPos int, QuyDirection uint8) (la LA) {
...
func findNextProEdge(eIDArr []DBG_MAX_INT, laArr []LA, gapRegArr []GapRegion, QuyPos int, QuyDirection uint8) (nLaArr []LA) {
	// direction := GetDirection(node, eID)
	// for i := 0; i < bnt.BaseTypeNum; i++ {
	// 	var nEID DBG_MAX_INT
	// 	if direction == FORWARD {
	// 		nEID = node.EdgeIDIncoming[i]
	// 	} else {
	// 		nEID = node.EdgeIDOutcoming[i]
...
func findNextProPath(subp [2][]DBG_MAX_INT, laArr []LA, gapRegArr []GapRegion, QuyPos int, QuyDirection uint8) int {
	idx := -1
...
func SumLen(eIDArr []DBG_MAX_INT, edgesArr []DBGEdge) (sl int) {
	sl = Kmerlen - 1
	for _, eID := range eIDArr {
...
func IsSubPath(srcArr, subArr []DBG_MAX_INT) (is bool) {
...
func GetNextShortPath(consistenceA [][]DBG_MAX_INT, i, j int) (ni, nj int) {
	idx := IndexEID(consistenceA[i], DBG_MAX_INT(i))
	if idx < j {
...
func IsInComing(eIDcoming [4]DBG_MAX_INT, eID DBG_MAX_INT) bool {
...
func GetNextConsisPath(edgesArr []DBGEdge, nodesArr []DBGNode, left []DBG_MAX_INT, consistenceA [][]DBG_MAX_INT) (n, m int) {
...
func GetNextConsistenceAPosition(consistenceA [][]DBG_MAX_INT, eID DBG_MAX_INT, direction uint8) (n, m int) {
...
func IsInLAArr(laArr []LA, eID DBG_MAX_INT) bool {
	for _, la := range laArr {
		if la.RefID == eID {
			return true
		}
...
func GetRemainArr(csArr []DBG_MAX_INT, eID, lasteID DBG_MAX_INT) (remainArr []DBG_MAX_INT) {
	idx := IndexEID(csArr, eID)
	if 0 < idx && idx < len(csArr)-1 {
		if lasteID == csArr[idx-1] {
			if len(csArr)-(idx+1) > 0 {
				remainArr = make([]DBG_MAX_INT, len(csArr)-(idx+1))
		...
		if idx == 0 {
			if lasteID != csArr[idx+1] {
				remainArr = make([]DBG_MAX_INT, len(csArr)-(1))
...
/*func GetNeedSortEID(edgesArr []DBGEdge, rc chan DBG_MAX_INT, numCPU int) {
	for _, e := range edgesArr {
...
/*func paraSortPathMat(edgesArr []DBGEdge, rc chan DBG_MAX_INT) {
...
func GetNextPathLA(eIDArr []DBG_MAX_INT, laArr []LA) (remainLaArr []LA) {
	for _, eID := range eIDArr {
		edgeLaArr := GetLARegion(laArr, eID)
		if len(edgeLaArr) > 0 {
...
func GetInterSection(arr1, arr2 []DBG_MAX_INT) (interSecArr []DBG_MAX_INT) {
	for _, e1 := range arr1 {
...
func CheckAndCutLocalCycle(eIDArr []DBG_MAX_INT) (cutArr []DBG_MAX_INT, cuted bool) {
	var cycleArr []DBG_MAX_INT
	cutArr = eIDArr
	i := len(eIDArr) - 2
	for i >= 0 {
...
func SetLAFlag(laArr []LA, eID DBG_MAX_INT) {
	for i, _ := range laArr {
...
func paraFindLongMappingPath(lac chan []LA, wc chan []DBG_MAX_INT, edgesArr []DBGEdge, nodesArr []DBGNode, RefIDMapArr []DBG_MAX_INT, consistenceA [][2][]DBG_MAX_INT) {
	for {
		laArr := <-lac
		if len(laArr) == 0 {
			var guardPath []DBG_MAX_INT
						...
						if len(eIDArr) == 1 {
							...
							if IsOverlapGapRegArr(gapRegArr, QuyPos, QuyDirection, len(edgesArr[eIDArr[0]].Utg.Ks)-Kmerlen) {
								...
								if (e1.StartNID == e2.StartNID && e1.EndNID == e2.EndNID) || (e1.StartNID == e2.EndNID && e1.EndNID == e1.StartNID) {
									...
									var arr [2][]DBG_MAX_INT
									for j := 0; j < 2; j++ {
										edge := edgesArr[eIDArr[j]]
										var nodeID DBG_MAX_INT
...
func GetOrderID(edgesArr []DBGEdge) (ReadIDMapArr []DBG_MAX_INT) {
	for _, e := range edgesArr {
		if e.ID > 0 {
...
func WriteLongPathToDBG(wc chan []DBG_MAX_INT, edgesArr []DBGEdge, numCPU int) [][]Path {
	...
	for {
		...
		if len(path) > 3 {
			if path[0] == DBG_MAX_INT(0) {
				path = path[1:]
			}
			if path[len(path)-1] == DBG_MAX_INT(0) {
...
func GetSameDirectionPath(pathArr []Path, eID DBG_MAX_INT) (sdPathArr []Path) {
...
func AllowZeroDeepEqual(arr1, arr2 []DBG_MAX_INT) bool {
	if arr1[0] != arr2[0] {
		return false
	}
	for i, _ := range arr1 {
...
func GetOverlapArr(arr []DBG_MAX_INT, pathArr []Path) (overlapMat []Path) {
	for _, p := range pathArr {
...
func GetTotalFreq(pathArr []Path, idx int, eID DBG_MAX_INT) (freqSum int) {
...
func CheckSignificantDiff(scoreArr []int, eIDArr []DBG_MAX_INT) (eID DBG_MAX_INT, ok bool) {
...
func GetMinCutPathArr(pathArr []Path, minPathFreq int) (consisPathArr []DBG_MAX_INT) {
	for i := 0; ; i++ {
		var eIDArr []DBG_MAX_INT
		var freqArr []int
		for _, p := range pathArr {
			if len(p.IDArr) > i {
...
func GetIndexDBG_MAX_INTArr(arr []DBG_MAX_INT, ele DBG_MAX_INT) int {
	idx := -1
	for i, id := range arr {
		if id == ele {
			idx = i
...
func GetFlankPath(crossPathA []Path, eID, neID DBG_MAX_INT) (fpArr []Path) {
...
/*func ParseUniquePath(edgesArr []DBGEdge, nodesArr []DBGNode, LongPathMatArr [][]Path) (mergeMat [][]DBG_MAX_INT) {
...
func StaticsMergeMat(matArr [][]DBG_MAX_INT, edgesArr []DBGEdge) {
