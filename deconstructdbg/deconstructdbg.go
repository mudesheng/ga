...
func WriteLongPathToFile(wc chan string, pathfn, ONTMapInfofn string, numCPU int) {
	...
	//pathArr.Arr = make([]*constructdbg.ReadPath, 0, 10000)
...
type EIDLen struct {
	...
	NextEID constructdbg.DBG_MAX_INT
	Length  int
}
...
func GetDirectionBubbleRepeatArr(e *constructdbg.DBGEdge, edgesArr []constructdbg.DBGEdge, direction uint8) (bubbleRepeatArr []uint32) {
	...
	for {
		...
		ne := &edgesArr[ea[0]]
		strand = constructdbg.GetNextEdgeStrand2(le, ne, strand, direction)
		le = ne
...
func SortByUniqueEdgeLen(edgesArr []constructdbg.DBGEdge, nodesArr []constructdbg.DBGNode, Haplotype bool, kmerlen int) (sortedEIDLenArr []EIDLen) {
	...
	for i := range edgesArr {
		...
		if e.GetBubbleRepeatFlag() > 0 {
			...
			eIDLen.EID = e.ID
			eIDLen.NextEID = e.ID
			var bubbleRepeatArr []constructdbg.DBG_MAX_INT
			...
			// BACKWARD
			{
				...
				nd := nodesArr[e.StartNID]
				for nd.ID > 0 {
					eIDArr := constructdbg.GetNextEIDArr(eID, nd)
					if len(eIDArr) == 0 {
						break
					}
					ne := edgesArr[eIDArr[0]]
					if ne.GetBubbleFlag() > 0 {
						eIDLen.Length += len(ne.Utg.Ks) - (kmerlen - 1)
					} else if ne.GetBubbleRepeatFlag() > 0 {
						eIDLen.Length += len(ne.Utg.Ks) - (kmerlen - 1)
						eIDLen.EID = ne.ID
					...
					if ne.StartNID == nd.ID {
						nd = nodesArr[ne.EndNID]
					} else {
						nd = nodesArr[ne.StartNID]
					}
					eID = ne.ID
				}
			}
...
/*func GetOverlapPath(matchPath [2][]constructdbg.DBG_MAX_INT, eID constructdbg.DBG_MAX_INT, readPathArr []*constructdbg.ReadPath, edgesPathRelation []int, direction uint8, minMapFreq int) (extendP [2][]constructdbg.DBG_MAX_INT, freq int) {
...
func GetPathSeqLen(pa []constructdbg.DBG_MAX_INT, edgesArr []constructdbg.DBGEdge, kmerlen int) (tl int) {
	for _, eID := range pa {
		tl += edgesArr[eID].GetSeqLen() - (kmerlen - 1)
	}
	if tl > 0 {
		tl += kmerlen - 1
	}
...
type MatchInfo struct {
	Path        constructdbg.Path
	EID         constructdbg.DBG_MAX_INT // path cen EID
	PathPos     int
	MatchSeqLen int
	DiffSeqLen  int
	ExtendLen   int
...
func ExtendPath(edgesArr []constructdbg.DBGEdge, nodesArr []constructdbg.DBGNode, readPathArr []constructdbg.ReadPath, edgesPathRelationArr [][]uint32, minMapingFreq, minMappingLen, kmerlen int, eIDLen EIDLen, MaxPathLen, MaxBubbleSeqLen int) constructdbg.Path {
	e := edgesArr[eIDLen.EID]
	MinExtendLen := 2000
	var p constructdbg.Path
	//minMapEdgesNum := 3
	p.IDArr = make([]constructdbg.DBG_MAX_INT, len(e.PathMat[0].IDArr))
	p.AltArr = make([]constructdbg.DBG_MAX_INT, len(e.PathMat[0].IDArr))
	...
	{
		idx := constructdbg.IndexEID(p.IDArr, eIDLen.EID)
		...
		for idx >= 0 && idx < len(p.IDArr) {
			...
			for ; i >= 0; i-- {
				...
				for j := 0; j < 2; j++ {
					...
					ep.IDArr = make([]constructdbg.DBG_MAX_INT, len(tp.IDArr))
					ep.AltArr = make([]constructdbg.DBG_MAX_INT, len(tp.IDArr))
					...
					if x < len(ep.IDArr)-1 && i < len(p.IDArr)-1 && ep.IDArr[x+1] != p.IDArr[i+1] {
						...
						constructdbg.ReverseDBG_MAX_INTArr(ep.AltArr)
						x = len(ep.IDArr) - 1 - x
					...
					if epPos, matchLen, diffLen, ok := EqualPath2(p, ep, e2.ID, constructdbg.BACKWARD, edgesArr, nodesArr, kmerlen, MaxPathLen, MaxBubbleSeqLen); ok {
						var mi MatchInfo
						mi.EID = e2.ID
						mi.Path = ep
			...
			if len(matchArr) > 0 {
				...
				if bestmi.Path.Freq >= minMapingFreq {
					...
					idx = constructdbg.IndexEID(p.IDArr, bestmi.EID)
	...
	// add right partion
	{
		idx := constructdbg.IndexEID(p.IDArr, eIDLen.NextEID)
		...
		for idx >= 0 && idx < len(p.IDArr) {
			...
			for ; i < len(p.IDArr); i++ {
				...
				for j := 0; j < 2; j++ {
					...
					if epPos, matchLen, diffLen, ok := EqualPath2(p, ep, e2.ID, constructdbg.FORWARD, edgesArr, nodesArr, kmerlen, MaxPathLen, MaxBubbleSeqLen); ok {
						var mi MatchInfo
						mi.EID = e2.ID
						mi.Path = ep
						mi.DiffSeqLen = diffLen
...
func FindMaxPath(sortedEIDLenArr []EIDLen, edgesArr []constructdbg.DBGEdge, nodesArr []constructdbg.DBGNode, readPathArr []constructdbg.ReadPath, edgesPathRelationArr [][]uint32, minMapingFreq, minMappingLen int, kmerlen int, Haplotype bool, MaxPathLen, MaxBubbleSeqLen int) (pA []constructdbg.Path) {
	...
	for _, item := range sortedEIDLenArr {
		...
		/*if Haplotype {
			if e.GetBubbleFlag() > 0 {
				ok := true
				if e.StartNID > 0 {
					seID := constructdbg.GetNextEID(e.ID, nodesArr[e.StartNID])
					if seID > 0 && len(edgesArr[seID].Utg.Ks) < bubbleNearEdgeMinLen {
						ok = false
					}
				}
				if e.EndNID > 0 {
					eeID := constructdbg.GetNextEID(e.ID, nodesArr[e.EndNID])
					if eeID > 0 && len(edgesArr[eeID].Utg.Ks) < bubbleNearEdgeMinLen {
						ok = false
					}
				}
				if ok == false {
...
func Transform2ReadPath(mi constructdbg.MapingInfo) (rp constructdbg.ReadPath) {
	rp.ReadID = mi.ID
...
func ConstructEdgesPathRelationship(edgesArr []constructdbg.DBGEdge, nodesArr []constructdbg.DBGNode, cs <-chan constructdbg.MapingInfo, mapRecordSize int) (readPathArr []constructdbg.ReadPath, edgesPathRelationArr [][]uint32) {
	readPathArr = make([]constructdbg.ReadPath, 0, mapRecordSize)
	edgesPathRelationArr = make([][]uint32, len(edgesArr))
	var mi constructdbg.MapingInfo
	var ok bool
	for {
		mi, ok = <-cs
		if !ok {
...
func CopyReadPathArr(readPathArr []*constructdbg.ReadPath, relationArr []int) [][2][]constructdbg.DBG_MAX_INT {
...
func CopyPathArr(pathArr [][2][]constructdbg.DBG_MAX_INT, relationArr []int) [][2][]constructdbg.DBG_MAX_INT {
	subArr := make([][2][]constructdbg.DBG_MAX_INT, len(relationArr))
	for i, idx := range relationArr {
		//path0, path1 := readPathArr[idx].GetPath0(), readPathArr[idx].GetPath1()
...
func IndexEID(arr []uint32, eID uint32) int {
	for i, id := range arr {
		if id == eID {
			return i
		}
...
func IndexBubbleRepeatArr(pathArr [2][]uint32, bubbleRepeatArr []uint32) (int, int) {
	...
	for i, id := range pathArr[0] {
		if id == eID1 {
			...
			idx := IndexEID(bubbleRepeatArr, id)
			if idx >= 0 {
				idxP = i
				idxB = idx
				break
...
func IsInDirectionBubbleRepeatPath(eID1, eID2 uint32, edgesArr []constructdbg.DBGEdge, direction uint8) bool {
	ok := false
	if direction == constructdbg.FORWARD {
...
func GetNextEIDArr(e *constructdbg.DBGEdge, strand bool, direction uint8) (arr []uint32) {
	if direction == constructdbg.FORWARD {
		if strand {
			arr = GetComingEdgeArr2(e.EdgeIDOutcoming)
		} else {
			arr = GetComingEdgeArr2(e.EdgeIDIncoming)
		}
	} else {
		if strand {
			arr = GetComingEdgeArr2(e.EdgeIDIncoming)
...
func checkEdgePathArrDirection(edgesArr []constructdbg.DBGEdge, ePathArr [][2][]uint32, bubbleRepeatArr []uint32) [][2][]uint32 {
	...
	if len(bubbleRepeatArr) > 1 {
		if IsInDirectionBubbleRepeatPath(eID, bubbleRepeatArr[1], edgesArr, constructdbg.FORWARD) {
			strand1 = constructdbg.PLUS
		} else {
			strand1 = constructdbg.MINUS
		}
		nextEID = bubbleRepeatArr[len(bubbleRepeatArr)-1]
		if IsInDirectionBubbleRepeatPath(nextEID, bubbleRepeatArr[len(bubbleRepeatArr)-2], edgesArr, constructdbg.BACKWARD) {
			strand2 = constructdbg.PLUS
		} else {
			strand2 = constructdbg.MINUS
		}
	} else {
		strand1 = constructdbg.PLUS
	}
	eIDLeftArr = GetNextEIDArr(e, strand1, constructdbg.BACKWARD)
	eIDRightArr = GetNextEIDArr(e, strand1, constructdbg.FORWARD)
	if len(bubbleRepeatArr) > 1 {
		ne := &edgesArr[nextEID]
		nextEIDLeftArr = GetNextEIDArr(ne, strand2, constructdbg.BACKWARD)
		nextEIDRightArr = GetNextEIDArr(ne, strand2, constructdbg.FORWARD)
	}
	...
	for i, extArr := range ePathArr {
		...
		for j, id := range extArr[0] {
			...
			idx1 := IndexEID(bubbleRepeatArr, uint32(te.ID))
			if idx1 < 0 {
				continue
			...
			if idx1 < len(bubbleRepeatArr)-1 {
				if IsInDirectionBubbleRepeatPath(bubbleRepeatArr[idx1], bubbleRepeatArr[idx1+1], edgesArr, constructdbg.FORWARD) {
					strand = constructdbg.PLUS
				} else {
					strand = constructdbg.MINUS
				}
			} else {
				if IsInDirectionBubbleRepeatPath(bubbleRepeatArr[idx1], bubbleRepeatArr[idx1-1], edgesArr, constructdbg.BACKWARD) {
					strand = constructdbg.PLUS
				} else {
					strand = constructdbg.MINUS
				}
			}
			eIDRightArr = GetNextEIDArr(te, strand, constructdbg.FORWARD)
			if j > 0 {
...
func GetIDFreqArrHaplotype(ePathArr [][2][]constructdbg.DBG_MAX_INT, j int, direction uint8, diffLen int, path, altPath []constructdbg.DBG_MAX_INT, UniqueScore int) (idFreqArr []IDFreq) {
	if direction == constructdbg.FORWARD {
		for k := 0; k < len(ePathArr); k++ {
			p := ePathArr[k]
			ok := true
			...
			if ok && len(p[0]) > j && len(p[1]) > j {
				if p[0][j] > 0 && p[1][j] == 0 {
					idFreqArr = AddIDFreqArr(idFreqArr, p[0][j], UniqueScore)
...
func AdjustAltPath(pm []constructdbg.DBG_MAX_INT, idx int, direction uint8, oldP, newP []constructdbg.DBG_MAX_INT) (path []constructdbg.DBG_MAX_INT) {
	if direction == constructdbg.BACKWARD {
		dl := len(newP) - len(oldP)
		if idx <= utils.MaxInt(len(newP), len(oldP)) {
			path = pm
			return
		}
		path = make([]constructdbg.DBG_MAX_INT, 0, len(pm))
		if dl > 0 {
		...
		if idx >= len(pm)-utils.MaxInt(len(newP), len(oldP)) {
			path = pm
			return
		}
		dl := len(newP) - len(oldP)
...
func ReplaceEPath(pm []constructdbg.DBG_MAX_INT, idx int, direction uint8, oldP, newP []constructdbg.DBG_MAX_INT) (path []constructdbg.DBG_MAX_INT) {
...
func FoundFirstEID(arr []constructdbg.DBG_MAX_INT) (idx int) {
	idx = -1
	for i, id := range arr {
		if id > 0 {
			idx = i
...
func FoundLastEID(arr []constructdbg.DBG_MAX_INT) (idx int) {
	idx = -1
...
func GetEPathSectionPath(pm []constructdbg.DBG_MAX_INT, idx int, direction uint8, mp []constructdbg.DBG_MAX_INT, diffLen, MaxPathLen int) (path []constructdbg.DBG_MAX_INT) {
	if direction == constructdbg.FORWARD {
		pl := MaxPathLen
		if pl > len(pm)-idx {
			pl = len(pm) - idx
		...
		if diffLen > 0 {
			...
			if reflect.DeepEqual(pm[idx-diffLen:idx], mp[len(mp)-diffLen:]) {
				path = make([]constructdbg.DBG_MAX_INT, pl)
		...
		if diffLen > 0 {
			...
			if reflect.DeepEqual(pm[idx+1:idx+1+diffLen], mp[:diffLen]) {
				path = constructdbg.GetReverseDBG_MAX_INTArr(pm[idx+1-pl : idx+1])
			}
		} else {
			path = constructdbg.GetReverseDBG_MAX_INTArr(pm[idx+1-pl : idx+1])
		}
...
func GetEPathMatPath(ePathMatArr [][2][]constructdbg.DBG_MAX_INT, idx int, direction uint8, mp []constructdbg.DBG_MAX_INT, eID constructdbg.DBG_MAX_INT, diffLen int) (path []constructdbg.DBG_MAX_INT) {
...
func GetShareDBGNode(nodesArr []constructdbg.DBGNode, le, ne *constructdbg.DBGEdge) (nd constructdbg.DBGNode) {
...
func GetShareDBGNID(le, ne *constructdbg.DBGEdge) (nID constructdbg.DBG_MAX_INT) {
	if le.StartNID == ne.StartNID || le.StartNID == ne.EndNID {
		nID = le.StartNID
	} else if le.EndNID == ne.StartNID || le.EndNID == ne.EndNID {
		nID = le.EndNID
...
func SortByFreq(pathArr []constructdbg.BubblePath, idFreqArr []IDFreq) []constructdbg.BubblePath {
	count := 0
	maxLen := 0
...
func IsInPathArr(path []constructdbg.DBG_MAX_INT, pathArr []constructdbg.BubblePath, edgesArr []constructdbg.DBGEdge, kmerlen int) (np []constructdbg.DBG_MAX_INT, ok bool) {
...
func CleanPathFlank(pm []constructdbg.DBG_MAX_INT, idx int, direction uint8) []constructdbg.DBG_MAX_INT {
	if direction == constructdbg.FORWARD {
...
func AdjustEPathMat2(ePathMatArr [][2][]constructdbg.DBG_MAX_INT, idx int, idFreqArr []IDFreq, direction uint8, edgesArr []constructdbg.DBGEdge, nodesArr []constructdbg.DBGNode, mp []constructdbg.DBG_MAX_INT, kmerlen int, diffLen int, MaxPathLen, MaxBubbleSeqLen int) {
	/*if direction == constructdbg.FORWARD {
		mp = mp[:len(mp)-1]
	...
	var pathArr []constructdbg.BubblePath
	...
	var path0, path1 []constructdbg.DBG_MAX_INT
	if e1.GetTwoEdgesCycleFlag() > 0 || e2.GetTwoEdgesCycleFlag() > 0 || e1.StartNID == e1.EndNID || e2.StartNID == e2.EndNID {
		...
		nd := GetShareDBGNode(nodesArr, le, &edgesArr[idFreqArr[0].ID])
		pa := constructdbg.GetNextPathArr(le, nd, MaxPathLen+1, MaxBubbleSeqLen, edgesArr, nodesArr, kmerlen)

		pathArr, _ = constructdbg.GetBubblePathArr(pa, edgesArr, kmerlen, MaxPathLen, MaxBubbleSeqLen)
...
func GetBubbleRepeatArr(e *constructdbg.DBGEdge, edgesArr []constructdbg.DBGEdge) (bubbleRepeatArr []uint32) {
	ea1 := GetDirectionBubbleRepeatArr(e, edgesArr, constructdbg.BACKWARD)
	ea2 := GetDirectionBubbleRepeatArr(e, edgesArr, constructdbg.FORWARD)
	...
	if len(ea1) > 0 && len(ea2) > 0 {
		...
		//bubbleRepeatArr = append(bubbleRepeatArr, constructdbg.GetReverseDBG_MAX_INTArr(ea1)...)
...
func GetPathRelationArr(rp constructdbg.ReadPath) [2][]uint32 {
	var extArr [2][]uint32
...
func GetBubbleRepeatReadPathArr(bubbleRepeatArr []uint32, edgesPathRelationArr [][]uint32, readPathArr []constructdbg.ReadPath) (ePathArr [][2][]uint32) {
...
func GetCorrelationEID(ePathArr [][2][]constructdbg.DBG_MAX_INT, eID1, eID2 constructdbg.DBG_MAX_INT, idx1, idx2 int) (correlationNum int) {
...
func GetHaplotypePath(ePathArr [][2][]constructdbg.DBG_MAX_INT, idx int, mp [2][]constructdbg.DBG_MAX_INT, direction uint8, edgesArr []constructdbg.DBGEdge, eID1, eID2 constructdbg.DBG_MAX_INT) (h1, h2 constructdbg.DBG_MAX_INT, ok bool) {
	ok = true
	if direction == constructdbg.FORWARD {
		px := -1
...
func DiffLenFirst(a1, a2 []constructdbg.DBG_MAX_INT, diffLen int) int {
	if len(a1) != len(a2) {
		fmt.Printf("[DiffLenFirst] len(a1): %v != len(a2): %v\n", len(a1), len(a2))
	}
	min := utils.MinInt(len(a1), len(a2))
	//max := utils.MaxInt(len(a1), len(a2))
	i := 0
	for ; i < min; i++ {
		if a1[i] != a2[i] {
			break
...
func DiffLenLast(a1, a2 []constructdbg.DBG_MAX_INT) int {
	if len(a1) != len(a2) {
...
func CheckPathConnectity(path constructdbg.Path, edgesArr []constructdbg.DBGEdge, nodesArr []constructdbg.DBGNode) constructdbg.Path {
	for i := 1; i < len(path.IDArr)-1; i++ {
		...
		if nd.ID > 0 {
			eIDArr := constructdbg.GetOtherEArr(nd, e.ID)
...
func IsBubblePath(leID constructdbg.DBG_MAX_INT, idFreqArr []IDFreq, edgesArr []constructdbg.DBGEdge, nodesArr []constructdbg.DBGNode, kmerlen int) (bubOk bool) {
	MaxPathLen := 10
	...
	pathArr := constructdbg.GetNextPathArr(le, nd, MaxPathLen+1, MaxBubbleSeqLen, edgesArr, nodesArr, kmerlen)
	/*for x, p := range pathArr {
		fmt.Printf("[IsBubblePath]pathArr[%v]: %v\n", x, p)
	}*/
	bubPathArr, _ := constructdbg.GetBubblePathArr(pathArr, edgesArr, kmerlen, MaxPathLen, MaxBubbleSeqLen)
	//fmt.Printf("[IsBubblePath]bubPathArr: %v\n", bubPathArr)
	var bubIDArr []constructdbg.DBG_MAX_INT
	for _, bp := range bubPathArr {
		bubIDArr = append(bubIDArr, bp.Path[0])
	}
	bubOk = true
	for _, idf := range idFreqArr {
		if constructdbg.IsInDBG_MAX_INTArr(bubIDArr, idf.ID) {
...
func DeleteLowFreq(idFreqArr []IDFreq, minMapFreq, MinDeleteFreqDiff int32) []IDFreq {
	y := 0
	for ; y < len(idFreqArr); y++ {
		if idFreqArr[y].Freq < minMapFreq {
			break
		}
		if y > 0 && idFreqArr[y].Freq*MinDeleteFreqDiff < idFreqArr[y-1].Freq {
			break
		}
	}
	idFreqArr = idFreqArr[:y]
	return idFreqArr
}
...
func ReAdjustEPathArr(ePathArr [][2][]uint32, eIDLen EIDLen, bubbleRepeatArr []uint32, edgesArr []constructdbg.DBGEdge, leftMax, al int) {
...
func GetPathEndIdx(pathArr [2][]uint32, bubbleRepeatArr []uint32, direction uint8, j int) (int, int) {
	...
	if direction == constructdbg.FORWARD {
		...
		if pathArr[1][j] > 0 {
			...
			idx := IndexEID(bubbleRepeatArr, pathArr[0][j])
			if idx >= 0 {
				return j + 1, idx
			}
		}
		for x := j + 1; x < end; x++ {
			eID := pathArr[0][x]
			idx := IndexEID(bubbleRepeatArr, eID)
			if idx >= 0 {
				return x, -1
			}
			if bub {
...
func FoundFirstGeID(graph []DAGEdge, gID int, extArr [2][]uint32, j int) (geID int) {
	...
	for x := j; x >= 0; {
		nID, nnID := GetNextNextGEID(graph, uint16(gID), extArr, x, constructdbg.BACKWARD)
		if nID >= 0 {
			geID = nID
			gID = nID
...
func GetBubbleArr(edgesArr []constructdbg.DBGEdge, eID1, eID2 uint32) (arr []uint32) {
	e1, e2 := &edgesArr[eID1], &edgesArr[eID2]
	for _, id := range e1.EdgeIDOutcoming {
		if id < 2 {
			continue
		}
		if constructdbg.IsInComing(e2.EdgeIDIncoming, id) || constructdbg.IsInComing(e2.EdgeIDOutcoming, id) {
...
func MergePathArr(edgesArr []constructdbg.DBGEdge, nodesArr []constructdbg.DBGNode, readPathArr []constructdbg.ReadPath, sortEIDLenArr []EIDLen, edgesPathRelationArr [][]uint32, opt Options, MaxPathLen, MaxBubbleSeqLen int) {
	for _, eIDLen := range sortEIDLenArr {
		...
		// Directed Acyclic Graph and Topological order
		{
			...
			for _, extArr := range ePathArr {
				...
				// FORWARD
				{
					...
					for j := idxP; j < len(extArr[0]); {
						...
						//fmt.Printf("j:%d\n", j)
						//idx := IndexEID(bubbleRepeatArr, extArr[0][j])
						//if idx >= 0 {
						//	graph[idx].Weight++
						//	j++
						//}
						end, idx := GetPathEndIdx(extArr, bubbleRepeatArr, constructdbg.FORWARD, j)
						if end-j >= len(ge.EIDArr) {
							if EqualUint32Arr(extArr[0][j:j+len(ge.EIDArr)], ge.EIDArr) {
								...
								graph = SplitEIDArrAddGraph(graph, geID, splitLen, constructdbg.FORWARD)
								ge = &graph[geID]
								j += splitLen
							}
						} else {
				...
				// BACKWARD
				{
					...
					for j := idxP + len(graph[geID].EIDArr) - 1; j >= 0; {
						...
						//	j++
						//}
						end, idx := GetPathEndIdx(extArr, bubbleRepeatArr, constructdbg.BACKWARD, j)
						//fmt.Printf("j:%d ge:%v\n", j, *ge)
						if j+1-end >= len(ge.EIDArr) {
							if EqualUint32Arr(extArr[0][j+1-len(ge.EIDArr):j+1], ge.EIDArr) {
								...
								graph = SplitEIDArrAddGraph(graph, geID, splitLen, constructdbg.BACKWARD)
								ge = &graph[geID]
								j -= len(ge.EIDArr)
							}
						} else {
			...
			for _, extArr := range ePathArr {
				...
				if idxB < 0 {
					...
					if e.GetBubbleRepeatFlag() > 0 {
						...
						geID, _ = GetNextNextGEID(graph, uint16(geID), extArr, start, constructdbg.FORWARD)
					}
				}
				// FORWARD
				gID := geID
				for j := start; j < len(extArr[0]); {
					...
					j += len(ge.EIDArr)
					nID, nnID := GetNextNextGEID(graph, uint16(gID), extArr, j, constructdbg.FORWARD)
				...
				if idxB == 0 {
					...
					for j := start + len(graph[gID].EIDArr) - 1; j >= 0; {
						...
						j -= len(ge.EIDArr)
						nID, nnID := GetNextNextGEID(graph, uint16(gID), extArr, j, constructdbg.BACKWARD)
						//fmt.Printf("j:%d nID:%d nnID:%d\n", j, nID, nnID)
						if nnID >= 0 {
							AddNextInfo(ge, uint16(nnID))
						} else {
			...
			if geID == 0 {
				if e.GetUniqueFlag() > 0 {
					path := GetPathArr(graph, uint16(geID), constructdbg.BACKWARD)
					ReverseUint32Arr(path)
					path = append(path, GetPathArr(graph, uint16(geID), constructdbg.FORWARD)...)
					p[0] = constructdbg.Transform2Path(path)
				} else if e.GetBubbleRepeatFlag() > 0 {
					for i, gID := range graph[geID].OutEdgesID {
						if i > 1 {
							break
						}
						path := GetPathArr(graph, gID, constructdbg.BACKWARD)
						ReverseUint32Arr(path)
						path = append(path, GetPathArr(graph, gID, constructdbg.FORWARD)...)
						p[i] = constructdbg.Transform2Path(path)
					}
				}
			} else if geID > 0 {
				for i, gID := range graph[geID].OutEdgesID {
					...
					path := GetPathArr(graph, gID, constructdbg.FORWARD)
					p[i] = constructdbg.Transform2Path(path)
				}
			}
			...
			for i := 0; i < 2; i++ {
				...
				pm.IDArr = constructdbg.Transform2Path(tp)
				for _, eID := range tp {
					e := &edgesArr[eID]
		...
		var mp [2][]constructdbg.DBG_MAX_INT
		var altmp [2][]constructdbg.DBG_MAX_INT
		...
		// FORWARD
		{
			...
			for j := leftMax + 1; j < al; j++ {
				...
				if ok0 {
					idFreqArr0 = GetIDFreqArrHaplotype(ePathArr, j, constructdbg.FORWARD, diffLen, mp[0], altmp[0], UniqueScore)
				}
				if diffLen > 0 && ok1 {
					idFreqArr1 = GetIDFreqArrHaplotype(ePathArr, j, constructdbg.FORWARD, diffLen, mp[1], altmp[1], UniqueScore)
				}
				...
				if len(idFreqArr0) <= 1 && len(idFreqArr1) <= 1 {
					if len(idFreqArr0) == 1 {
						...
						//AdjustEPathMat2(ePathArr, j, idFreqArr0[0].ID, constructdbg.FORWARD, edgesArr, mp[0], kmerlen, diffLen)
					}
					if len(idFreqArr1) == 1 {
						...
						//AdjustEPathMat2(ePathArr, j, idFreqArr1[0].ID, constructdbg.FORWARD, edgesArr, mp[1], kmerlen, diffLen)
					} else if opt.Haplotype {
						mp[1] = append(mp[1], idFreqArr0[0].ID)
						if freq1 > idFreqArr0[0].Freq {
							freq1 = idFreqArr0[0].Freq
						}
						//AdjustEPathMat2(ePathArr, j, idFreqArr0[0].ID, constructdbg.FORWARD, edgesArr, mp[1], kmerlen, diffLen)
					}
				} else {
					if diffLen == 0 {
						if idFreqArr0[0].Freq > idFreqArr0[1].Freq*5 {
							...
							if e1.GetTwoEdgesCycleFlag() > 0 || e2.GetTwoEdgesCycleFlag() > 0 || e1.StartNID == e1.EndNID || e2.StartNID == e2.EndNID {
								...
								//AdjustEPathMat2(ePathArr, j, idFreqArr1[1].ID, constructdbg.FORWARD, edgesArr, mp[1], kmerlen, diffLen)
							} else if IsBubblePath(mp[0][len(mp[0])-1], idFreqArr0, edgesArr, nodesArr, opt.Kmer) {
								dl := len(mp[0]) - len(altmp[0])
								ta := make([]constructdbg.DBG_MAX_INT, dl)
				...
				//adjust EPathMat
				{
					...
					if num0 > 1 {
						AdjustEPathMat2(ePathArr, j, idFreqArr0[:num0], constructdbg.FORWARD, edgesArr, nodesArr, mp[0], opt.Kmer, diffLen, MaxPathLen, MaxBubbleSeqLen)
					}

					if num1 > 1 {
						AdjustEPathMat2(ePathArr, j, idFreqArr1[:num1], constructdbg.FORWARD, edgesArr, nodesArr, mp[1], opt.Kmer, diffLen, MaxPathLen, MaxBubbleSeqLen)
					}
				}
			}
		}
		...
		// BACKWARD
		{
			forwardDiffLen := utils.AbsInt(len(mp[0]) - len(mp[1]))
			//freq := math.MaxInt32
			ok0, ok1 := true, true
			for j := leftMax - 1; j >= 0; j-- {
				...
				var idFreqArr0, idFreqArr1 []IDFreq
				if ok0 {
					idFreqArr0 = GetIDFreqArrHaplotype(ePathArr, j, constructdbg.BACKWARD, diffLen, mp[0], altmp[0], UniqueScore)
				}
				if diffLen > 0 && ok1 {
					idFreqArr1 = GetIDFreqArrHaplotype(ePathArr, j, constructdbg.BACKWARD, diffLen, mp[1], altmp[1], UniqueScore)
				}
				...
				if len(idFreqArr0) <= 1 && len(idFreqArr1) <= 1 {
					var n0, n1 []constructdbg.DBG_MAX_INT
					if len(idFreqArr0) == 1 {
						...
						//AdjustEPathMat2(ePathArr, j, idFreqArr0[0].ID, constructdbg.BACKWARD, edgesArr, mp[0], kmerlen, diffLen)
					}
					if len(idFreqArr1) == 1 {
						...
						//AdjustEPathMat2(ePathArr, j, idFreqArr1[0].ID, constructdbg.BACKWARD, edgesArr, mp[1], kmerlen, diffLen)
					} else if opt.Haplotype {
						...
						//AdjustEPathMat2(ePathArr, j, idFreqArr0[0].ID, constructdbg.BACKWARD, edgesArr, mp[1], kmerlen, diffLen)
					}
				} else {
					if diffLen == 0 {
						if idFreqArr0[0].Freq > idFreqArr0[1].Freq*5 {
							...
							var n0 []constructdbg.DBG_MAX_INT
							n0 = append(n0, idFreqArr0[0].ID)
							n0 = append(n0, mp[0]...)
							mp[0] = n0
							if freq0 > idFreqArr0[0].Freq {
							...
							e1, e2 := edgesArr[idFreqArr0[0].ID], edgesArr[idFreqArr0[1].ID]
							if e1.GetTwoEdgesCycleFlag() > 0 || e2.GetTwoEdgesCycleFlag() > 0 || e1.StartNID == e1.EndNID || e2.StartNID == e2.EndNID {
								var n0, alt []constructdbg.DBG_MAX_INT
								dl := len(mp[0]) - len(altmp[0])
								n0 = append(n0, idFreqArr0[0].ID)
								n0 = append(n0, mp[0]...)
								mp[0] = n0
						...
						if len(idFreqArr1) == 1 {
							var n1 []constructdbg.DBG_MAX_INT
							n1 = append(n1, idFreqArr1[0].ID)
							n1 = append(n1, mp[1]...)
							mp[1] = n1
							if freq1 > idFreqArr1[0].Freq {
							...
							fmt.Fprintf(os.Stderr, "[MergePathArr]diffLen: %v, eIDLen: %v, idx: %d has low quality edge: {eID1[%d] : Freq[%d]}, {eID2[%d] : Freq[%d]}\n", diffLen, eIDLen, j, idFreqArr1[0].ID, idFreqArr1[0].Freq, idFreqArr1[1].ID, idFreqArr1[1].Freq)
							var n1, alt []constructdbg.DBG_MAX_INT
				...
				//adjust EPathMat
				{
					...
					if num0 > 1 {
						AdjustEPathMat2(ePathArr, j, idFreqArr0[:num0], constructdbg.BACKWARD, edgesArr, nodesArr, mp[0], opt.Kmer, diffLen, MaxPathLen, MaxBubbleSeqLen)
					}

					if num1 > 1 {
						AdjustEPathMat2(ePathArr, j, idFreqArr1[:num1], constructdbg.BACKWARD, edgesArr, nodesArr, mp[1], opt.Kmer, diffLen, MaxPathLen, MaxBubbleSeqLen)
					}
				}
			}
		}
		...
		if len(mp[0]) >= 3 {
			...
			if len(bubbleRepeatArr) > 1 {
				ne := edgesArr[eIDLen.NextEID]
				idx2 := constructdbg.IndexEID(mp[0], eIDLen.NextEID)
				if idx2 >= 0 {
			...
			if len(mp[1]) > 0 && reflect.DeepEqual(mp[0], mp[1]) == false {
				...
				if len(bubbleRepeatArr) > 1 {
					ne := edgesArr[eIDLen.NextEID]
					idx2 := constructdbg.IndexEID(mp[1], eIDLen.NextEID)
					if idx2 >= 0 {
			...
			for j := leftMax; j >= 0; j-- {
				...
				if y == 0 {
					...
					if te.GetTwoEdgesCycleFlag() == 0 && j < leftMax && j > 0 && len(idFreqArr) > 1 {
						AdjustEPathMat2(ePathArr, j, idFreqArr, constructdbg.BACKWARD, edgesArr,nodesArr, mp, opt.Kmer,0, MaxPathLen, MaxBubbleSeqLen)
					}
				} else {
					break
				}
			...
			for j := leftMax + 1; j < leftMax+rightMax; j++ {
				...
				if y == 0 {
					...
					if te.GetTwoEdgesCycleFlag() == 0 && j < leftMax+rightMax-1 && len(idFreqArr) > 1 {
						AdjustEPathMat2(ePathArr, j, idFreqArr, constructdbg.FORWARD, edgesArr, mp[leftLen:], opt.Kmer)
					}
				} else {
					break
				}
...
func ExtractSeq(edgesArr []constructdbg.DBGEdge, nodesArr []constructdbg.DBGNode, joinPathArr []constructdbg.Path, DcDBGEdgesfn string, kmerlen int) {
	...
	for _, p := range joinPathArr {
		...
		buffp.Write(constructdbg.Transform2Char(seq))
		buffp.WriteString("\n")
...
func SimplifyByLongReadsPath(edgesArr []constructdbg.DBGEdge, nodesArr []constructdbg.DBGNode, cs <-chan constructdbg.MapingInfo, opt Options, MaxPathLen, MaxBubbleSeqLen, mapRecordSize int) (mergePathArr []constructdbg.Path) {
...
func GetKmerInfo(bs uint64, ID uint32, pos uint64, strand bool) (ki KmerInfo) {
	//ki.SetKmer(CompressToUint64(ks))
	ki.Kmer = bs
	ki.Kmer = ki.Kmer<<SeqPositionBitNum | pos
	ki.Kmer <<= 1
	if strand == constructdbg.PLUS {
		ki.Kmer |= 0x1
	}
	ki.ID = ID
	//fmt.Printf("[GetKmerInfo]ki.Kmer:%b\n", ki.GetKmer())
	return
}
...
func GetSeqMiniKmerInfoArr(ks []byte, buf, kmerInfoArr []KmerInfo, ID uint32, SeedLen, WinSize int) []KmerInfo {
	if len(ks) < SeedLen+WinSize {
		var t []KmerInfo
		return t
	}
	kmerInfoArr = kmerInfoArr[:0]
	//kmerInfoArr := make([]KmerInfo, 0, len(ks)/WinSize*2)
	//rs = constructdbg.GetReverseCompByteArr2(ks, rs)
	...
	rs := constructcf.GetReverseCompletUint64(bs, uint32(SeedLen))
	fmt.Printf("[GetSeqMiniKmerInfoArr]bs:%X\n", bs)
	fmt.Printf("[GetSeqMiniKmerInfoArr]rs:%X\n", rs)
	ki := GetKmerInfo(bs, ID, 0, constructdbg.PLUS)
	rki := GetKmerInfo(rs, ID, 0, constructdbg.MINUS)
	if bs < rs {
		buf[idx] = ki
	} else {
		buf[idx] = rki
	...
	for i := SeedLen; i < sl; i++ {
		bs <<= bnt.NumBitsInBase
		bs |= uint64(ks[i])
		bs &= MUSK //rki.GetKmer() &= MUSK
		ki = GetKmerInfo(bs, ID, uint64(i-SeedLen-1), constructdbg.PLUS)

		rs >>= bnt.NumBitsInBase
		//bt = uint64(rs[sl-i-SeedLen])
		//fmt.Printf("[GetSeqMiniKmerInfos]bt:%b\n", bt<<BITNUM)
		rs |= (uint64(bnt.BntRev[ks[i]]) << BITNUM)
		rki = GetKmerInfo(rs, ID, uint64(i-(SeedLen-1)), constructdbg.MINUS)
		if bs < rs {
			buf[idx] = ki
		} else {
			buf[idx] = rki
		...
		//fmt.Printf("[GetSeqMiniKmerInfos]i:%d rb:%b ki.Kmer:%b rki.Kmer:%b\n", i, rs[sl-i-SeedLen], ki.GetKmer(), rki.GetKmer())
		if idx < WinSize {
			continue
...
func RadixSortKIArr(kmerArr []KmerInfo, bulkArr [][]KmerInfo, SeedLen, BaseNum, NumBulk uint64) ([]KmerInfo, [][]KmerInfo) {
	if len(kmerArr) < 2 {
	...
	for i := uint64(0); i < (SeedLen+BaseNum-1)/BaseNum; i++ {
		...
		for _, ki := range kmerArr {
			idx := (ki.GetKmer() >> (i * BaseNum * bnt.NumBitsInBase)) & MUSK
...
func RadixSortKIArr3Bit(kmerArr []KmerInfo, bulkArr [][]KmerInfo, SeedLen, BitNum, NumBulk uint64) ([]KmerInfo, [][]KmerInfo) {
	if len(kmerArr) < 2 {
		return kmerArr, bulkArr
	}
	if len(kmerArr) < 2000 {
	...
	overflowNum := 0
	countArr := make([]int, NumBulk)
	sz := (SeedLen*bnt.NumBitsInBase + BitNum - 1) / BitNum
...
func IsRepeatEdge(e *constructdbg.DBGEdge) bool {
...
func IsUniqueEdge(e *constructdbg.DBGEdge) bool {
	if e.GetUniqueFlag() > 0 || e.GetBubbleFlag() > 0 || e.GetTwoEdgesCycleFlag() > 0 {
		return true
	}
	return false
}

func IsRepeatEdgeNoDB(e *constructdbg.DBGEdge, kmerlen int) bool {
...
func IsEdgeNoDB(e *constructdbg.DBGEdge, kmerlen int) bool {
	if IsRepeatEdge(e) {
...
func IndexEdgeFreqArr(freqArr []FreqInfo, WinSize int, pos uint32) int {
	idx := -1
	step := WinSize * 4 / 7
	x := int(pos) / step
	if x >= len(freqArr) {
...
func SortDBGEdgesKmer(dbgCS chan<- DBGKmerInfoPac, edgesArr []constructdbg.DBGEdge, SeedLen, WinSize, HighFreqKmerMin, HighFreqKmerMax, MinimerKmerNum, kmerlen int) {
	var dbgKmerInfoPac DBGKmerInfoPac
	var deleteKmerNum int
	var edgeNum, mapEdgeNum int
	MinEdgeLen = (kmerlen - 1) * 3 / 2
	kmerArr := make([]KmerInfo, 0, MinimerKmerNum)
	bufSize := WinSize * 3
	buf := make([]KmerInfo, bufSize)
	ka := make([]KmerInfo, 5000)
	//rs := make([]byte, 10000)
	for i := 0; i < len(edgesArr); i++ {
		...
		ka = GetSeqMiniKmerInfoArr(e.Utg.Ks, buf, ka, uint32(e.ID), SeedLen, WinSize)
		if IsEdgeNoDB(e, kmerlen) {
			sort.Sort(KIArr(ka))
			e.SeedInfoArr = make([]constructdbg.SeedInfo, len(ka))
			for x, ki := range ka {
				e.SeedInfoArr[x] = constructdbg.SeedInfo(ki.Kmer)
			}
	...
	bulkArr := make([][]KmerInfo, NumBulk)
	for i := range bulkArr {
		bulkArr[i] = make([]KmerInfo, len(kmerArr)/int(NumBulk)*3/2)
	}
	...
	//sortKmerArr := make([]KmerInfo, count)
...
func GetReadArrSeqLen(ra []constructcf.ReadInfo) (sl int) {
	for _, ri := range ra {
		sl += len(ri.Seq)
	}
	return
...
func SortLongReadsKmer(rc <-chan []constructcf.ReadInfo, SeedLen, WinSize int, buckReadsNum int, cs chan<- ReadsBucketInfo, finishT chan<- int) {
	...
	bulkArr := make([][]KmerInfo, NumBulk)
	for i := range bulkArr {
		bulkArr[i] = make([]KmerInfo, buckReadsNum*10000/int(NumBulk)*3/2)
	}
	ka := make([]KmerInfo, 5000)
	for {
		ra := <-rc
		...
		kmerArr := make([]KmerInfo, 0, tl*2/WinSize)
		seedInfoMat := make([][]constructdbg.SeedInfo, len(ra))
		for i, ri := range ra {
			...
			kmerArr = append(kmerArr, ka...)
			seedInfoArr := make([]constructdbg.SeedInfo, len(ka))
			for x, ki := range ka {
				seedInfoArr[x] = constructdbg.SeedInfo(ki.Kmer)
			}
...
//const KmerResetStrand = (1 << (SeedLen*bnt.NumBitsInBase + SeqPositionBitNum)) - 1
...
func (m *MapingKmerInfo) GetKmer() uint64 {
	return m.Kmer >> (SeqPositionBitNum + 1)
}
func (m *MapingKmerInfo) SetKmer(kmer uint64) {
	m.Kmer = (kmer << (SeqPositionBitNum + 1)) | (m.Kmer & KmerMask)
}

func (m *MapingKmerInfo) GetPosition() uint64 {
	return (m.Kmer >> 1) & PositionMask
}
func (m *MapingKmerInfo) SetPosition(p uint64) {
	m.Kmer = (p << 1) | (m.Kmer & KmerReSetPosition)
}
...
func (m *MapingKmerInfo) GetMapStrand() bool {
	if (m.Kmer & 0x1) == uint64(m.Info&0x1) {
		return true
	} else {
		return false
	}
...
func (m *MapingKmerInfo) SetStrand(s bool) {
	if s == constructdbg.PLUS {
		m.Kmer |= 0x1
	} else {
		m.Kmer = m.Kmer & KmerResetStrand
	}
}
...
type ReadsBucketInfo struct {
	...
	SeedInfoMat [][]constructdbg.SeedInfo
	KmerSortArr []KmerInfo
}

func LoadLongReads(ONTFnbr string, rc chan<- []constructcf.ReadInfo, buckReadsNum int, processT chan<- int) {
	...
	go constructcf.ReadBrFile(ONTFnbr, fncs1, bufSize)
	go constructcf.GetReadFileRecord(fncs1, recordChan1, format, bufSize, true)
	readInfoArr := make([]constructcf.ReadInfo, 0, 1000)
...
func LoadLongReadsAndSort(cfgInfo constructcf.CfgInfo, NumCPU int, cs chan<- ReadsBucketInfo, SeedLen, WinSize int, buckReadsNum int) {
...
func GetInterSectionKmerInfo(dbgKmerInfoPac DBGKmerInfoPac, kmerInfoArrB []KmerInfo, BLen int, StartID uint32, SeedLen uint16) (bucketInterSecKmerInfoArr [][]MapingKmerInfo) {
...
func GetInterSectionKmerInfoB(dbgKmerInfoPac DBGKmerInfoPac, kmerInfoArrB []KmerInfo, StartID uint32, SeedLen uint16, bucketInterSecKmerInfoArr [][]MapingKmerInfo) [][]MapingKmerInfo {
	//startReadID := uint32(rb.ReadsArr[0].ID)
...
func GetInterSectionKmerInfo2(kmerInfoArrA, kmerInfoArrB []KmerInfo, BLen int, StartID uint32, SeedLen uint16) (bucketInterSecKmerInfoArr [][]MapingKmerInfo) {
	//startReadID := uint32(rb.ReadsArr[0].ID)
	bucketInterSecKmerInfoArr = make([][]MapingKmerInfo, BLen)
	for i := 0; i < BLen; i++ {
...
func GetEdgesKIArr(kb []MapingKmerInfo, edgesArr []constructdbg.DBGEdge, MinKmerScore, LongEdgeMinLen int) (edgesKIArr []EdgeKI) {
...
func GetMKIDistance(cen, mk MapingKmerInfo, edgesArr []constructdbg.DBGEdge, distanceE int) (dX, dY int) {
	if mk.GetStrand() == constructdbg.PLUS {
		if cen.GetPosition() <= mk.GetPosition() {
			...
			if cen.EID != mk.EID {
				dY = len(edgesArr[mk.EID].Utg.Ks) - (int(mk.GetEPosition()) + int(mk.GetMapLen())) + distanceE
			} else {
...
func GetTwoBlockScore(cen, mk MapingKmerInfo, GapCost int, edgesArr []constructdbg.DBGEdge, distanceE int) (sc int) {
	var gapLen int
...
func BlockToNearBlocksScoreArrCrossEdges(kb []MapingKmerInfo, Idx int, strand bool, visitArr []bool, GapCost, MaxGapLen int, SeqType uint8, bsA []int, direction uint8, flankLen, kmerlen int, edgesArr []constructdbg.DBGEdge, SeedLen uint16, distanceE int) {
	cen := kb[Idx]
	// BACKWARD
	for i := Idx - 1; i >= 0; i-- {
		mk := kb[i]
		...
		if strand == constructdbg.PLUS {
			if mk.EID != cen.EID {
				dY := distanceE + (len(edgesArr[mk.EID].Utg.Ks) - (int(mk.GetEPosition()) + int(mk.GetMapLen())))
	...
	if direction == constructdbg.FORWARD {
		if strand == constructdbg.PLUS {
			for _, arr := range sharePat {
				...
				if constructdbg.GetNextEdgeStrand(e, ne, nodesArr, strand) == nstrd {
					sharePat[idx] = arr
					idx++
				}
			}
...
func GetMaxChainBlockArr(kb []MapingKmerInfo, maxPathInfoArr []MaxPathInfo, edgesArr []constructdbg.DBGEdge, nodesArr []constructdbg.DBGNode, SeedLen, GapCost, MaxGapLen, kmerlen int, SeqType uint8, LongEdgeMinLen int, MinChainScoreIdentityPercent, seedNumPercent float32, logfp io.Writer) []MaxPathInfo {
	...
	for i := 0; i < len(kb); {
		...
		min := utils.MinInt(readRegLen, edgeRegLen)
		if el >= LongEdgeMinLen {
			if float32(maxPI.Score) > float32(LongEdgeMinLen)*MinChainScoreIdentityPercent && utils.AbsInt(readRegLen-edgeRegLen) < min/10 && min > LongEdgeMinLen*3/4 {
				maxPI.Base = uint32(i)
				maxPI.EID = eID
				maxPI.Kb = kb
			...
			if float32(maxPI.Score) > float32(el)*MinChainScoreIdentityPercent && utils.AbsInt(readRegLen-edgeRegLen) < min/10 && min > el/2 {
				maxPI.Base = uint32(i)
				maxPI.EID = eID
				maxPI.Kb = kb
				maxPathInfoArr = append(maxPathInfoArr, maxPI)
	...
	/*for i, mpi := range maxPathInfoArr {
		...
		fmt.Printf("[GetMaxChainBlockArr] maxPathInfoArr[%v]: Edge ID: %v, EdgeLen: %v, Read region[%v---%v] Len:%v, Edge region[%v---%v], score: %v, Percent: %v\n", i, mpi.Arr[0].EID, len(edgesArr[mpi.Arr[0].EID].Utg.Ks), start, end, end-start, mpi.Arr[0].GetEPosition(), mpi.Arr[len(mpi.Arr)-1].GetEPosition(), mpi.Score, mpi.Score*100/(int(end)-int(start)))
	}*/
				...
				for {
					...
					if ok, bd := GetReadBound(pIArr, constructdbg.BACKWARD); ok {
						...
						if count == 1 {
							...
							var ok bool
							bsArr, ok = CheckMaxScoreBound(pi.Arr[len(pi.Arr)-1].GetPosition(), pi.Score, constructdbg.BACKWARD, bsArr)
							if ok {
								continue
							} else {
								break˝
							...
							for n, p := range pIArr {
								if p.Score == maxSc {
									...
									var ok bool
									bsArr, ok = CheckMaxScoreBound(mpi.Arr[len(mpi.Arr)-1].GetPosition(), mpi.Score, constructdbg.BACKWARD, bsArr)
									if ok {
				...
				for {
					...
					if ok, bd := GetReadBound(pIArr, constructdbg.FORWARD); ok {
						...
						if count == 1 {
							...
							//pi.DistE = pIArr[idx].DistE
							var ok bool
							bsArr, ok = CheckMaxScoreBound(pi.Arr[len(pi.Arr)-1].GetPosition()+uint32(pi.Arr[len(pi.Arr)-1].GetMapLen()), pi.Score, constructdbg.FORWARD, bsArr)
							if ok {
								continue
							} else {
								break
							...
							for n, p := range pIArr {
								if p.Score == maxSc {
									...
									//distR, distE := CheckBound(mpi.Arr, ReadLen, constructdbg.FORWARD)
									var ok bool
									bsArr, ok = CheckMaxScoreBound(mpi.Arr[len(mpi.Arr)-1].GetPosition()+uint32(mpi.Arr[len(mpi.Arr)-1].GetMapLen()), mpi.Score, constructdbg.FORWARD, bsArr)
									if ok {
...
func GetEdgeRegionSeq(e constructdbg.DBGEdge, mpi MaxPathInfo) (edgeRegionSeq []byte) {
	...
	//fmt.Printf("[GetEdgeRegionSeq]eID: %v, arr[0] ReadPos: %v, EdgePos: %v, strand: %v, Len: %v, arr[last] ReadPos: %v, EdgePos: %v, strand: %v, Len: %v, len(edge): %v\n", e.ID, mpi.Arr[0].GetPosition(), mpi.Arr[0].GetEPosition(), mpi.Arr[0].GetStrand(), mpi.Arr[0].GetMapLen(), last.GetPosition(), last.GetEPosition(), last.GetStrand(), last.GetMapLen(), len(e.Utg.Ks))
	if strand == constructdbg.PLUS {
...
func GetDirectionEdgesSeq(edgesArr []constructdbg.DBGEdge, nodesArr []constructdbg.DBGNode, kmerlen int, strand bool, path []constructdbg.DBG_MAX_INT) (edgesSeq []byte) {
	extendLen := kmerlen - 1
	...
	for i, eID := range path {
		...
		if strand == constructdbg.PLUS {
			edgesSeq = append(edgesSeq, ne.Utg.Ks[startPos:]...)
		} else {
			edgesSeq = append(edgesSeq, ne.Utg.Ks[:ne.GetSeqLen()-startPos]...)
			constructdbg.ReverseCompByteArr(edgesSeq[seqLen:])
		}
		e = ne
...
func GetDirectionEdgesSeqLen(edgesArr []constructdbg.DBGEdge, nodesArr []constructdbg.DBGNode, kmerlen int, strand bool, startPos, extendLen int, path []constructdbg.DBG_MAX_INT, eID constructdbg.DBG_MAX_INT) (edgesSeq []byte) {
	...
	seqLen := 0
	if strand == constructdbg.PLUS {
		edgesSeq = append(edgesSeq, e.Utg.Ks[startPos:]...)
	} else {
		edgesSeq = append(edgesSeq, e.Utg.Ks[startPos:]...)
		constructdbg.ReverseCompByteArr(edgesSeq[seqLen:])
	}
	...
	for i := 1; i < len(path); i++ {
		...
		ne := &edgesArr[eID]
		strand = constructdbg.GetNextEdgeStrand(e, ne, nodesArr, strand)
		extLen := len(ne.Utg.Ks) - (int(kmerlen) - 1)
		...
		if strand == constructdbg.PLUS {
			edgesSeq = append(edgesSeq, ne.Utg.Ks[kmerlen-1:kmerlen-1+extLen]...)
		} else {
			edgesSeq = append(edgesSeq, ne.Utg.Ks[ne.GetSeqLen()-(kmerlen-1)-extLen:ne.GetSeqLen()-(kmerlen-1)]...)
			constructdbg.ReverseCompByteArr(edgesSeq[seqLen:])
		}
		if finished {
			break
...
func GetDirectionEdgesSeqRegion(edgesArr []constructdbg.DBGEdge, nodesArr []constructdbg.DBGNode, direction uint8, kmerlen uint32, extendLen int, path []constructdbg.DBG_MAX_INT, anchor MapingKmerInfo) (edgesSeq []byte) {
	...
	if direction == constructdbg.FORWARD {
		if eri.Strand == constructdbg.PLUS {
	...
	for i := 1; i < len(path); i++ {
		eID := path[i]
		ne := &edgesArr[eID]
		eri.Strand = constructdbg.GetNextEdgeStrand(e, ne, nodesArr, eri.Strand)
		eri.EID = eID
		extLen := len(ne.Utg.Ks) - (int(kmerlen) - 1)
		if seqLen+extLen >= extendLen {
			finished = true
	...
	for _, eri := range edgeRegInfoArr {
		if eri.Strand == constructdbg.PLUS {
			edgesSeq = append(edgesSeq, edgesArr[eri.EID].Utg.Ks[eri.Start:eri.End]...)
		} else {
			edgesSeq = append(edgesSeq, edgesArr[eri.EID].Utg.Ks[eri.Start:eri.End]...)
			constructdbg.ReverseCompByteArr(edgesSeq[seqLen:])
		}
...
/*func GetDirectionEdgesSeq(edgesArr []constructdbg.DBGEdge, nodesArr []constructdbg.DBGNode, direction uint8, kb []MapingKmerInfo, kmerlen, extendLen int) (edgesSeq []byte) {
	...
	if direction == constructdbg.FORWARD {
		...
		if mkS.GetStrand() == constructdbg.PLUS {
			edgesSeq = append(edgesSeq, e.Utg.Ks[mkS.GetEPosition():]...)
		} else {
			edgesSeq = append(edgesSeq, constructdbg.GetReverseCompByteArr(e.Utg.Ks[:mkS.GetEPosition()+uint32(mkS.GetMapLen())])...)
		}
		...
		if mkE.GetStrand() == constructdbg.PLUS {
			...
			if int(mkE.GetEPosition()) > len(e.Utg.Ks)-(kmerlen-1) {
				dl := int(mkE.GetEPosition()) - (len(e.Utg.Ks) - (kmerlen - 1))
				edgesSeq = edgesSeq[:len(edgesSeq)-dl]
			} else {
		...
		var finished bool
		if mkS.GetStrand() == constructdbg.PLUS {
			edgesSeq = append(edgesSeq, constructdbg.GetReverseByteArr(e.Utg.Ks[:mkS.GetEPosition()+uint32(mkS.GetMapLen())])...)
		} else {
			edgesSeq = append(edgesSeq, constructdbg.GetCompByteArr(e.Utg.Ks[mkS.GetEPosition():])...)
		}
...
func GetEdgesSeq(edgesArr []constructdbg.DBGEdge, nodesArr []constructdbg.DBGNode, kb []MapingKmerInfo, kmerlen int) (edgesSeq []byte, path []constructdbg.DBG_MAX_INT, chainArr []Chain) {
	...
	// end Anthor
	{
		e := edgesArr[mkE.EID]
		if mkE.GetStrand() == constructdbg.PLUS {
			edgesSeq = append(edgesSeq, e.Utg.Ks[kmerlen-1:mkE.GetEPosition()+uint32(mkE.GetMapLen())]...)
		} else {
			edgesSeq = append(edgesSeq, constructdbg.GetReverseCompByteArr(e.Utg.Ks[mkE.GetEPosition():len(e.Utg.Ks)-(kmerlen-1)])...)
		}
		path = append(path, e.ID)
	}
...
func GetEdgeComing(e constructdbg.DBGEdge, direction uint8, strand bool, nodesArr []constructdbg.DBGNode, coming uint8) uint8 {
	...
	if constructdbg.IsInComing(nd.EdgeIDIncoming, e.ID) {
		coming = constructdbg.FORWARD
	} else {
		coming = constructdbg.BACKWARD
	}
	return coming
}

func GetNearDirectionEdgeIDArr(e constructdbg.DBGEdge, direction uint8, strand bool, coming uint8, nodesArr []constructdbg.DBGNode) (eArr []constructdbg.DBG_MAX_INT) {
	var nd constructdbg.DBGNode
	if strand == constructdbg.PLUS {
		if direction == constructdbg.FORWARD {
			if e.EndNID <= 0 {
	...
	c := (coming > 0)
	eArr = constructdbg.GetNearEdgeIDArr(nd, e.ID, c)
	return
}

func GetMappingDBGExtendInfo(mpi, extendMPI MaxPathInfo, score, flankLen uint32, strand bool, direction uint8, eLen, distanceE, kmerlen int, edgesArr []constructdbg.DBGEdge) (uint32, uint32) {
	same := false
	var fl int
		...
		if !same {
			...
			if i == 0 {
				...
				if min < 0-int(lastMKI.GetMapLen()) {
					...
					/*if min < 0 {
						extendMpArr[i].SetRPos(extendMpArr[i].GetPosition() + uint32(utils.AbsInt(min)))
						if extendMpArr[i].GetStrand() == constructdbg.PLUS {
							extendMpArr[i].SetEPos(extendMpArr[i].GetEPosition() + uint32(utils.AbsInt(min)))
						}
						extendMpArr[i].GetMapLen() -= uint16(utils.AbsInt(min))
					}*/
		...
		if !same {
			...
			if i == 0 {
				...
				if min < 0-int(lastMKI.GetMapLen()) {
					...
					/*if min < 0 {
						if extendMpArr[i].GetStrand() == constructdbg.MINUS {
							extendMpArr[i].SetEPos(extendMpArr[i].GetEPosition() + uint32(utils.AbsInt(min)))
						}
						extendMpArr[i].GetMapLen() -= uint16(utils.AbsInt(min))
					}*/
...
func GetMKI2EdgeDistance(extendEPI ExtendEdgePathInfo, neID constructdbg.DBG_MAX_INT, lastMki MapingKmerInfo, direction uint8, edgesArr []constructdbg.DBGEdge, kmerlen int) (distance int) {
	if direction == constructdbg.FORWARD {
		if lastMki.EID == uint32(neID) {
			if lastMki.GetStrand() == constructdbg.PLUS {
				distance = 0 - (int(lastMki.GetEPosition()) + int(lastMki.GetMapLen()))
			} else {
				distance = int(lastMki.GetEPosition()) + int(lastMki.GetMapLen()) - len(edgesArr[lastMki.EID].Utg.Ks)
			}
		} else {
			//idx := constructdbg.GetIndexDBG_MAX_INTArrDirection(path, constructdbg.DBG_MAX_INT(neID), constructdbg.BACKWARD)
			epi := extendEPI
			for epi.EID > 0 {
				eID := epi.EID
				if uint32(eID) != lastMki.EID {
					distance += len(edgesArr[eID].Utg.Ks) - (kmerlen - 1)
				} else {
					if lastMki.GetStrand() == constructdbg.PLUS {
						distance += len(edgesArr[eID].Utg.Ks) - (kmerlen - 1) - (int(lastMki.GetEPosition()) + int(lastMki.GetMapLen()))
					} else {
...
/*func GetExtendMapingKmerInfoArr(ekb []MapingKmerInfo, ekbIdx int, strand bool, direction uint8, edgesArr []constructdbg.DBGEdge, SeedLen uint16) (extendMpArr []MapingKmerInfo) {
	anchor := ekb[ekbIdx]
...
func ExtendPathChain(mpi MaxPathInfo, extendEPI ExtendEdgePathInfo, flankLen uint32, kmerlen int, direction uint8, ne constructdbg.DBGEdge, kbIdx int, strand bool, score uint32, edgesArr []constructdbg.DBGEdge, nodesArr []constructdbg.DBGNode, SeedLen uint16, maxPathInfoArr []MaxPathInfo, LongEdgeMinLen, MinChainScoreIdentityPercent int) (nmpi MaxPathInfo, newScore, fl uint32) {
	...
	if len(path) > 1 && edgesArr[leID].GetTwoEdgesCycleFlag() > 0 {
		//lastPathPos := constructdbg.IndexLastEID(path, constructdbg.DBG_MAX_INT(lastMki.EID))
	...
	if !mpiArrOk {
		...
		if len(ekb) == 0 {
			...
			fl = flankLen + uint32(len(ne.Utg.Ks)-(kmerlen-1))
			return
		}
	...
	if len(extendMPI.Arr) > 0 {
		...
		newScore, fl = GetMappingDBGExtendInfo(mpi, extendMPI, score, flankLen, strand, direction, len(ne.Utg.Ks), distanceE, int(kmerlen), edgesArr)
		if newScore > score {
			nmpi = extendMPI
		} else {
			nmpi = mpi
	...
	/*//startRPos := int(lastMki.GetPosition()) + int(lastMki.GetMapLen())
		for i := idx + 1; i < len(kb); i++ {
			...
			a := constructdbg.Min(min, int(mki.GetMapLen()))
			var b float64
			if gapLen > 1 {
			...
			sc = constructdbg.Min(a-int(b), int(nc.GetMapLen()))
		...
		for i := idx - 1; i >= 0; i-- {
			mki := kb[i]
			if int(mki.GetPosition()) < startRPos-(flankLen+len(ne.Utg.Ks)-(kmerlen-1)+len(ne.Utg.Ks)/10) {
				break
			}
			if mki.EID != uint32(ne.ID) || strand != mki.GetStrand() {
				continue
...
func SharePathLen(path, path1 []constructdbg.DBG_MAX_INT, edgesArr []constructdbg.DBGEdge, kmerlen, MaxPathLen, MaxBubbleSeqLen int) (sl int) {
	i, j := 0, 0
	//min := utils.MinInt(len(path), len(path1))
	for i < len(path) && j < len(path1) {
		if path[i] != path1[i] {
			if i > 0 && j > 0 {
				var pathArr [][]constructdbg.DBG_MAX_INT
				pathArr = append(pathArr, path[i-1:], path1[j-1:])
				bubPathArr, num := constructdbg.GetBubblePathArr(pathArr, edgesArr, kmerlen, MaxPathLen, MaxBubbleSeqLen)
				if num == 2 {
...
func GetExtendPathInfoPath(extendEPI ExtendEdgePathInfo) (path []constructdbg.DBG_MAX_INT) {
	path = make([]constructdbg.DBG_MAX_INT, 0, 10)
	epi := extendEPI
	...
	constructdbg.ReverseDBG_MAX_INTArr(path)
	return path
}

func GetBestAlignmentPath(bestArr []ExtendPathInfo, direction uint8, edgesArr []constructdbg.DBGEdge, nodesArr []constructdbg.DBGNode, read constructcf.ReadInfo, SeedLen, kmerlen, minMapLen, MaxPathLen, MaxBubbleSeqLen, WinSize int, maxPathInfoArr []MaxPathInfo, anchor MapingKmerInfo) (bestEPI ExtendPathInfo) {
...
func GetBestEPathInfo(highQualEPathInfoArr []ExtendPathInfo, edgesArr []constructdbg.DBGEdge, nodesArr []constructdbg.DBGNode, read constructcf.ReadInfo, SeedLen, kmerlen int, direction uint8, MaxPathLen, MaxBubbleSeqLen, WinSize int, maxPathInfoArr []MaxPathInfo, maxScoreArr []ScorePos, anchor MapingKmerInfo, ScoreWinSize int) (bestEPI ExtendPathInfo) {
...
func MuskBubble(eArr []constructdbg.DBG_MAX_INT, stk []ExtendPathInfo) ([]ExtendPathInfo, int) {
	var deleteNum int
	if len(stk) < 2 {
...
func MuskDiffLenBubble(le *constructdbg.DBGEdge, eArr []constructdbg.DBG_MAX_INT, stk []ExtendPathInfo, edgesArr []constructdbg.DBGEdge, nodesArr []constructdbg.DBGNode, kmerlen, MaxPathLen, MaxBubbleSeqLen int) ([]ExtendPathInfo, int) {
	var deleteNum int
	...
	pathArr := constructdbg.GetNextPathArr(le, nd, MaxPathLen+1, max*11/10, edgesArr, nodesArr, kmerlen)
	_, num := constructdbg.GetBubblePathArr(pathArr, edgesArr, kmerlen, MaxPathLen, MaxBubbleSeqLen)
	if num != len(pathArr) {
		return stk, deleteNum
	}
...
func DeleteUnmapPath(bestEPI ExtendPathInfo, direction uint8, edgesArr []constructdbg.DBGEdge, kmerlen int) ExtendPathInfo {
	...
	for ; i > 0; i-- {
		...
		if uint32(e.ID) == eID {
			if len(e.Utg.Ks)-fl < kmerlen*2 || e.GetBubbleFlag() > 0 {
				i--
			}
			break
		}
...
func MergeTwoFlankMapingPath(bestEPIB, bestEPIF ExtendPathInfo, mpi MaxPathInfo, eID uint32) (longRMInfo LongReadMappingInfo) {
	path := make([]constructdbg.DBG_MAX_INT, 0, len(bestEPIB.Path)+len(bestEPIF.Path)+1)
	path1 := make([]constructdbg.DBG_MAX_INT, 0, len(bestEPIB.Path)+len(bestEPIF.Path)+1)
...
func BiggerThanMaxScoreArr(maxScoreArr []ScorePos, startPos int, sa ScorePos, extendLen int, direciton uint8, ScoreWinSize int) bool {
	...
	if direciton == constructdbg.FORWARD {
...
func GetUniqueMaxPathInfo(maxPathInfoArr []MaxPathInfo, kmerlen, SeedLen, ReadLen int, edgesArr []constructdbg.DBGEdge, MinChainScoreIdentityPercent int) (uniqueMaxPathInfo MaxPathInfo) {
...
func GetMPIRegionScore(mpi *MaxPathInfo, start, end uint32) (sc float32) {
	...
	if strand == constructdbg.PLUS {
		for _, idx := range mpi.Arr {
			...
			maxS := utils.MaxUint32(start, s)
			minE := utils.MinUint32(end, e)
			if minE <= maxS {
...
func IsSameDBGNode(e, e2 constructdbg.DBGEdge, strand, strand2 bool) bool {
	if strand == strand2 {
...
func FilterMPIArr(maxPathInfoArr []MaxPathInfo, kmerlen, ReadLen int, edgesArr []constructdbg.DBGEdge, logfp io.Writer) []MaxPathInfo {
	// filter low mapping quality edge and filter only maping middle region of edge
	boundLen := kmerlen
	diff := 10
	filterFlag := make([]bool, len(maxPathInfoArr))
...
func GetAnchorMPIIdx(maxPathInfoArr []MaxPathInfo, priorIdxArr []uint16, edgesArr []constructdbg.DBGEdge) int {
	anchorIdx := -1
	maxScore := float32(0)
...
func GetMPIArrPrior(highQMPIArr []MaxPathInfo, kmerlen int, edgesArr []constructdbg.DBGEdge, nodesArr []constructdbg.DBGNode) ([]MaxPathInfo, int, []uint16) {
	// prior choose unique edge or bubble and bubbleRepeat edge
	/*if priorIdxArrLen < 5 {
		priorIdxArrLen = 5
	}*/
...
func PrintMPI(mpi *MaxPathInfo, edgesArr []constructdbg.DBGEdge) string {
	var s string
	if mpi.LenArr == 0 {
		return s
	}
...
func IsRepeatInEPIPath(bubArr []constructdbg.DBG_MAX_INT) bool {
...
func GetEdgeMappingStrand(eID constructdbg.DBG_MAX_INT, strd bool, path []constructdbg.DBG_MAX_INT, direction uint8, edgesArr []constructdbg.DBGEdge, nodesArr []constructdbg.DBGNode) (strand bool) {
	strand = strd
	e := &edgesArr[path[0]]
	ok := false
	if eID == path[0] {
...
func GetTwoEdgeCycleArr(path []constructdbg.DBG_MAX_INT, edgesArr []constructdbg.DBGEdge, nodesArr []constructdbg.DBGNode, cycleSize, TwoEdgeCyleMaxLen int) (cyclePathArr []CyclePath) {
	for i, eID := range path {
		e := edgesArr[eID]
		if e.GetTwoEdgesCycleFlag() > 0 && SumTwoEdgeCycleLen(e, edgesArr, nodesArr) < TwoEdgeCyleMaxLen {
			...
			neID := constructdbg.GetOtherEdgeInTwoEdgesCycle(edgesArr, nodesArr, eID)
			var cp CyclePath
			cp.EID = eID
			cp.Idx = i
...
func GetEqualPath(path []constructdbg.DBG_MAX_INT, bubPathArr []constructdbg.BubblePath) (idx int) {
	idx = -1
...
func GetDiffPathLenBubbleArr(path []constructdbg.DBG_MAX_INT, edgesArr []constructdbg.DBGEdge, nodesArr []constructdbg.DBGNode, MaxPathLen, MaxBubbleSeqLen int, kmerlen int) (diffLenBubArr []BubblePath) {
...
func GetExtendFlank(le constructdbg.DBGEdge, direction uint8, strand bool, flankExtendLen int) (startPos, extLen int) {
...
func BubbleAlignment(epi ExtendPathInfo, edgesArr []constructdbg.DBGEdge, nodesArr []constructdbg.DBGNode, rd constructcf.ReadInfo, SeedLen, kmerlen int, direction uint8, TwoEdgeCyleMaxLen, MaxPathLen, MaxBubbleSeqLen, WinSize int, maxPathInfoArr MaxPathInfoArr) ExtendPathInfo {
	// Get bubble edges
	...
	// add two edge cycle seq
	{
		...
		for i, cp := range cyclePathArr {
			...
			for _, id := range cp.Path {
				extLen += len(edgesArr[id].Utg.Ks) - (kmerlen - 1)
			}
			if len(epi.Path) > cp.Idx+1 {
				ne := edgesArr[epi.Path[cp.Idx+1]]
				if len(ne.Utg.Ks)-(kmerlen-1) > flankExtendLen {
					extLen += flankExtendLen
				} else {
					extLen += len(ne.Utg.Ks) - (kmerlen - 1)
				}
			}
			if DebugModel {
			...
			/*if direction == constructdbg.BACKWARD {
				es = constructdbg.GetReverseByteArr(es)
			}*/
	...
	// add diff path length  Bubble Path seq
	{
		...
		for i, bp := range diffLenBubArr {
			...
			std := constructdbg.GetNextEdgeStrand(le, ne, nodesArr, strand)
	...
	for i := 0; i < len(cyclePathArr); {
		...
		if len(bestCPArr) == 1 {
			...
			if len(cp.Path) != len(cyclePathArr[i].Path) {
				diffLen += utils.AbsInt(len(cp.Path) - len(cyclePathArr[i].Path))
			}
			epi.Path = p0
			epi.Path1 = p1
		}
	...
	// process diff edge length of bubble
	{
		...
		for i := 0; i < len(diffLenBubArr); {
			...
			idx := constructdbg.IndexEIDAllowDiffLen(epi.Path, db.Path[0], db.Idx, diffLen)
			if idx < 0 {
			...
			if len(bestDBArr) == 1 {
				...
				tp := make([]constructdbg.DBG_MAX_INT, len(bp.Path))
				p1 = append(p1, tp...)
				...
				if len(db.Path) != len(bp.Path) {
					diffLen += utils.AbsInt(len(db.Path) - len(bp.Path))
				}
				epi.Path = p0
				epi.Path1 = p1
				...
				tp := make([]constructdbg.DBG_MAX_INT, len(bp0.Path))
				tp[0] = bp1.Path[0]
				...
				if len(db.Path) != len(bp0.Path) {
					diffLen += utils.AbsInt(len(db.Path) - len(bp0.Path))
				}
				epi.Path = p0
				epi.Path1 = p1
			}
...
func SumTwoEdgeCycleLen(ne constructdbg.DBGEdge, edgesArr []constructdbg.DBGEdge, nodesArr []constructdbg.DBGNode) (sum int) {
	oeID := constructdbg.GetOtherEdgeInTwoEdgesCycle(edgesArr, nodesArr, ne.ID)
	sum = ne.GetSeqLen() + edgesArr[oeID].GetSeqLen()
	return
}

func GetNoSeedPathLen(epi ExtendPathInfo, edgesArr []constructdbg.DBGEdge, kmerlen int) int {
	fl := int(epi.FlankLen)
	pl := 0 // path length
...
func DeleteLowNumMinimersEdge(edgesArr []constructdbg.DBGEdge, edgeMiniKmerNumMap map[uint32]uint32, LongEdgeMinLen, MinChainScoreIdentityPercent int) map[uint32]uint32 {
	...
	for k, v := range edgeMiniKmerNumMap {
		...
		num1, num2 := GetKmerNum(v, constructdbg.PLUS), GetKmerNum(v, constructdbg.MINUS)
		num := utils.MaxInt(int(num1), int(num2))
		el := e.GetSeqLen()
		if el > LongEdgeMinLen {
...
func DeleteLowNumMinimersEdgePercent(edgesArr []constructdbg.DBGEdge, edgeMiniKmerNumMap map[uint32]uint32, LongEdgeMinLen, MinChainScoreIdentityPercent int) map[uint32]uint32 {
	//eIDArr := make([]uint32, 0, 1000)
...
func DeleteLowSeedNumMinimersEdge(SeedNumCountArr []SeedNumInfo, LongEdgeMinLen uint32, WinSize int, kmerlen, MQ int) ([]EdgeStrand, float32) {
	...
	for i, sni := range SeedNumCountArr {
		...
		num := utils.MaxUint8(numP, numM)
		SeedNumCountArr[i].SeedNumP = 0
		SeedNumCountArr[i].SeedNumM = 0
		totalNum++
		//num1, num2 := GetKmerNum(ct, constructdbg.PLUS), GetKmerNum(ct, constructdbg.MINUS)
		//el := sni.GetEdgeLen()
		...
		if num > 20 {
			...
			es.EdgeID = uint32(i)
			if num == numP {
				es.Strand = constructdbg.PLUS
			} else {
				es.Strand = constructdbg.MINUS
			}
...
func AddHighFreqKmer(ka []MapingKmerInfo, dbgKmerInfoPac DBGKmerInfoPac, edgesArr []constructdbg.DBGEdge, MinChainScoreIdentityPercent, SeedLen, LongEdgeMinLen, readLen int) []MapingKmerInfo {
	...
	for _, mki := range ka {
		...
		if mki.GetStrand() == constructdbg.PLUS {
			edgeMiniKmerNumMap[mki.EID] = SetKmerNum(edgeMiniKmerNumMap[mki.EID], constructdbg.PLUS)
		} else {
			edgeMiniKmerNumMap[mki.EID] = SetKmerNum(edgeMiniKmerNumMap[mki.EID], constructdbg.MINUS)
		}
	}
	...
	for _, mki := range ka {
		if mki.EID == math.MaxUint32 {
			...
			for _, eID := range eIDArr {
				v := SetKmerNum(edgeMiniKmerNumMap[eID], constructdbg.PLUS)
				edgeMiniKmerNumMap[eID] = SetKmerNum(v, constructdbg.MINUS)
			}
		}
	}
	...
	for _, mki := range ka {
		if mki.EID == math.MaxUint32 {
			...
			for _, eID := range eInfoArr {
				...
				//v := SetKmerNum(SeedNumCountArr[eID], constructdbg.PLUS)
			}*/
		} else {
...
func GetEdgeLowFreqSeedNumArr(edgesArr []constructdbg.DBGEdge, LongEdgeMinLen int) []SeedNumInfo {
...
func IsTipEdge(e constructdbg.DBGEdge) bool {
...
func FindONTReadsPath(dbgKmerInfoPac DBGKmerInfoPac, cs <-chan ReadsBucketInfo, SeedLen, winSize, GapCost, MaxGapLen, kmerlen int, wc chan<- LongReadMappingInfo, edgesArr []constructdbg.DBGEdge, nodesArr []constructdbg.DBGNode, SeqType uint8, MaxPathLen, MaxBubbleSeqLen, MinChainScoreIdentityPercent int) {
	extendLen := 3 * kmerlen
	...
	for {
		...
		for j, ka := range bucketInterSecKmerInfoArr {
			...
			//fmt.Printf("[FindONTReadsPath] Read region[%v---%v], readLen: %v, Edge region[%v---%v], EdgeLen: %v, score: %v\n", mpi.Arr[0].GetPosition(), last.GetPosition()+uint32(last.GetMapLen()), rl, mpi.Arr[0].GetEPosition(), last.GetEPosition(), len(edgesArr[eID].Utg.Ks), mpi.Score)
			...
			// read FORWARD mapping
			{
				...
				if pos+extendLen < rl {
					...
					epi.ExtendEPI.EID = constructdbg.DBG_MAX_INT(eID)
					//epi.Path = append(epi.Path, constructdbg.DBG_MAX_INT(id))
					epi.SetStrandFlag(anchor.GetStrand())
					var coming uint8
					epi.SetComingFlag(GetEdgeComing(edgesArr[id], constructdbg.FORWARD, epi.GetStrandFlag(), nodesArr, coming))
					if anchor.GetStrand() == constructdbg.PLUS {
						epi.ExtendLen = uint32(len(edgesArr[id].Utg.Ks)) - uint32(anchor.GetEPosition())
						epi.FlankLen = uint32(len(edgesArr[id].Utg.Ks) - (int(anchor.GetEPosition()) + int(anchor.GetMapLen())))
					} else {
					...
					for {
						...
						if BiggerThanMaxScoreArr(maxScoreArr, startPos, sa, etLen, constructdbg.FORWARD, ScoreWinSize) == false {
							continue
						}
						// check if has NGS Path
						{
							...
							if len(le.NGSPathArr) == 1 {
								path := constructdbg.EdgeFreqArr2DBG_MAX_INTArr(le.NGSPathArr[0])
								index := constructdbg.IndexEID(path, id)
								var np []constructdbg.DBG_MAX_INT
								if t.GetStrandFlag() == constructdbg.PLUS {
									if len(path)-index >= 3 {
										np = path[index+1:]
									}
								} else {
									if index >= 2 {
										np = constructdbg.GetReverseDBG_MAX_INTArr(path[:index])
									}
								}
								...
								if len(np) >= 2 && edgesArr[np[0]].GetTwoEdgesCycleFlag() == 0 && edgesArr[np[len(np)-1]].GetTwoEdgesCycleFlag() == 0 && edgesArr[np[len(np)-1]].StartNID != edgesArr[np[len(np)-1]].EndNID {
									...
									for _, eID := range np {
										...
										ne := &edgesArr[eID]
										strand := constructdbg.GetNextEdgeStrand(le, ne, nodesArr, t.GetStrandFlag())
										nt.ExtendEPI.EID = eID
										nt.ExtendEPI.Last = &t.ExtendEPI
										//nt.Path = make([]constructdbg.DBG_MAX_INT, len(t.Path)+1)
										//copy(nt.Path, t.Path)
										...
										if enIdx < 0 {
											nt.MPI = t.MPI
											nt.Score = t.Score
											nt.FlankLen = t.FlankLen + uint32(len(ne.Utg.Ks)-(kmerlen-1))
										} else {
											kbIdx := enArr[enIdx].Num
											nt.MPI, nt.Score, nt.FlankLen = ExtendPathChain(t.MPI, nt.ExtendEPI, t.FlankLen, kmerlen, constructdbg.FORWARD, *ne, int(kbIdx), strand, t.Score, edgesArr, nodesArr, uint16(SeedLen), maxPathInfoArr, LongEdgeMinLen, MinChainScoreIdentityPercent)
										}
										nt.SetComingFlag(GetEdgeComing(edgesArr[eID], constructdbg.FORWARD, strand, nodesArr, t.GetComingFlag()))
										nt.ExtendLen += t.ExtendLen + uint32(len(ne.Utg.Ks)-(kmerlen-1))
										var sp ScorePos
										...
										if BiggerThanMaxScoreArr(maxScoreArr, start, sp, etLen, constructdbg.FORWARD, ScoreWinSize) {
											if nt.Score > t.Score {
												maxScoreArr, _ = AddedToMaxScoreArrMPI(maxScoreArr, startPos, nt.MPI, int(nt.Score), constructdbg.FORWARD, ScoreWinSize)
											}
										} else {
											ok = false
											break
						...
						id := t.ExtendEPI.EID
						e := &edgesArr[id]
						eArr := GetNearDirectionEdgeIDArr(*e, constructdbg.FORWARD, t.GetStrandFlag(), t.GetComingFlag(), nodesArr)
						if DebugModel {
						...
						for _, eID := range eArr {
							...
							var nt ExtendPathInfo
							strand := constructdbg.GetNextEdgeStrand(e, ne, nodesArr, t.GetStrandFlag())
							//nt.Path = make([]constructdbg.DBG_MAX_INT, len(t.Path)+1)
							//copy(nt.Path, t.Path)
							//nt.Path[len(nt.Path)-1] = eID
							nt.ExtendEPI.EID = eID
							...
							if enIdx < 0 {
								...
								kbIdx := enArr[enIdx].Num
								nt.MPI, nt.Score, nt.FlankLen = ExtendPathChain(t.MPI, nt.ExtendEPI, t.FlankLen, kmerlen, constructdbg.FORWARD, *ne, int(kbIdx), strand, t.Score, edgesArr, nodesArr, uint16(SeedLen), maxPathInfoArr, LongEdgeMinLen, MinChainScoreIdentityPercent)
							} //else if constructdbg.IsSelfCycleEdge(edgesArr, nodesArr,ne.ID) && ne.GetSeqLen() < 500 {
							...
							sp.Pos = uint32(mkib.GetPosition()) + mkib.GetMapLen()
							sp.Score = uint32(nt.Score)
							if CheckExtend(nt, MinChainScoreIdentityPercent) && BiggerThanMaxScoreArr(maxScoreArr, startPos, sp, etLen, constructdbg.FORWARD, ScoreWinSize) {
								p := int(sp.Pos)
					...
					bestEPIF = GetBestEPathInfo(highQualEPathInfoArr, edgesArr, nodesArr, rb.ReadsArr[j], SeedLen, kmerlen, constructdbg.FORWARD, MaxPathLen, MaxBubbleSeqLen, winSize, maxPathInfoArr, maxScoreArr, anchor, ScoreWinSize)
					if len(bestEPIF.Path) == 0 {
						bestEPIF.Path = GetExtendPathInfoPath(bestEPIF.ExtendEPI)
					}

					if len(bestEPIF.Path) > 1 {
						bestEPIF = BubbleAlignment(bestEPIF, edgesArr, nodesArr, rb.ReadsArr[j], SeedLen, kmerlen, constructdbg.FORWARD, TwoEdgeCyleMaxLen, MaxPathLen, MaxBubbleSeqLen, winSize, maxPathInfoArr)
					}
					if DebugModel {
			...
			// read BACKWARD mapping
			{
				...
				if pos-extendLen > 0 {
					...
					epi.SetStrandFlag(anchor.GetStrand())
					var coming uint8
					epi.SetComingFlag(GetEdgeComing(edgesArr[id], constructdbg.BACKWARD, epi.GetStrandFlag(), nodesArr, coming))
					if anchor.GetStrand() == constructdbg.PLUS {
						epi.ExtendLen = anchor.GetEPosition() + uint32(anchor.GetMapLen())
						epi.FlankLen = anchor.GetEPosition()
					} else {
						epi.ExtendLen = uint32(len(edgesArr[id].Utg.Ks)) - anchor.GetEPosition()
					...
					for {
						...
						if BiggerThanMaxScoreArr(maxScoreArr, startPos, sa, etLen, constructdbg.BACKWARD, ScoreWinSize) == false {
							continue
						}
								...
								if len(np) >= 2 && edgesArr[np[0]].GetTwoEdgesCycleFlag() == 0 && edgesArr[np[len(np)-1]].GetTwoEdgesCycleFlag() == 0 && edgesArr[np[len(np)-1]].StartNID != edgesArr[np[len(np)-1]].EndNID {
									...
									for _, eID := range np {
										...
										if enIdx < 0 {
											...
											kbIdx := enArr[enIdx].Num
											nt.MPI, nt.Score, nt.FlankLen = ExtendPathChain(t.MPI, nt.ExtendEPI, t.FlankLen, kmerlen, constructdbg.BACKWARD, *ne, int(kbIdx), strand, t.Score, edgesArr, nodesArr, uint16(SeedLen), maxPathInfoArr, LongEdgeMinLen, MinChainScoreIdentityPercent)
										}
										nt.SetComingFlag(GetEdgeComing(edgesArr[eID], constructdbg.BACKWARD, strand, nodesArr, t.GetComingFlag()))
										nt.ExtendLen += t.ExtendLen + uint32(len(ne.Utg.Ks)-(kmerlen-1))
										var sp ScorePos
										...
										sp.Score = uint32(nt.Score)
										if BiggerThanMaxScoreArr(maxScoreArr, startPos, sp, etLen, constructdbg.BACKWARD, ScoreWinSize) {
											if nt.Score > t.Score {
												maxScoreArr, _ = AddedToMaxScoreArrMPI(maxScoreArr, startPos, nt.MPI, int(nt.Score), constructdbg.BACKWARD, ScoreWinSize)
											}
										} else {
											ok = false
											break
						...
						id := t.ExtendEPI.EID
						e := &edgesArr[id]
						eArr := GetNearDirectionEdgeIDArr(*e, constructdbg.BACKWARD, t.GetStrandFlag(), t.GetComingFlag(), nodesArr)
						if DebugModel {
						...
						for _, eID := range eArr {
							...
							sp.Score = uint32(nt.Score)
							//fmt.Printf("[FindONTReadsPath]sp: %v, maxScoreArr: %v\n", sp, maxScoreArr)
							if CheckExtend(nt, MinChainScoreIdentityPercent) && BiggerThanMaxScoreArr(maxScoreArr, startPos, sp, etLen, constructdbg.BACKWARD, ScoreWinSize) {
								p := int(sp.Pos)
								if nt.FlankLen > MaxEnlongationSeqLen {
									if p < MaxFlankLen {
										highQualEPathInfoArr = append(highQualEPathInfoArr, nt)
					...
					// process highQualEPathInfoArr
					bestEPIB = GetBestEPathInfo(highQualEPathInfoArr, edgesArr, nodesArr, rb.ReadsArr[j], SeedLen, kmerlen, constructdbg.BACKWARD, MaxPathLen, MaxBubbleSeqLen, winSize, maxPathInfoArr, maxScoreArr, anchor, ScoreWinSize)
					if len(bestEPIB.Path) == 0 {
						bestEPIB.Path = GetExtendPathInfoPath(bestEPIB.ExtendEPI)
					}
					if len(bestEPIB.Path) > 1 {
						bestEPIB = BubbleAlignment(bestEPIB, edgesArr, nodesArr, rb.ReadsArr[j], SeedLen, kmerlen, constructdbg.BACKWARD, TwoEdgeCyleMaxLen, MaxPathLen, MaxBubbleSeqLen, winSize, maxPathInfoArr)
					}
					if DebugModel {
...
/*
func FindDBGPath(highQMPIArr []MaxPathInfo, priorIdxArr []uint16, anchorIdx int, edgesArr []constructdbg.DBGEdge, nodesArr []constructdbg.DBGNode, kmerlen int, ri constructcf.ReadInfo) {
	...
	tipBoundNum := 0
	path := make([]constructdbg.DBG_MAX_INT, 0, 5)
	// BACKWARD
	mpi := anchorMPI
	mpiIdx := anchorIdx
	...
	for start > kmerlen*2 {
		if strand == constructdbg.PLUS {
			...
			nd := nodesArr[e.StartNID]
			coming = GetEdgeComing(e, constructdbg.BACKWARD, strand, nodesArr, coming)
			eArr := GetNearDirectionEdgeIDArr(e, constructdbg.BACKWARD, strand, coming, nodesArr)
			eIDArr := GetShareEID(eArr, highQMPIArr,mpiIdx, priorIdxArr, idx, constructdbg.BACKWARD, edgesArr)
...
func GetRegionSeedInfoArr(seedInfoArr []constructdbg.SeedInfo, start, end, readLen uint64) []constructdbg.SeedInfo {
	...
	sIA := make([]constructdbg.SeedInfo, ei+1-si)
	copy(sIA, seedInfoArr[si:ei+1])
	return sIA
}

func GetEdgeInterSectionKmerInfo(edgeSeedInfoArr, readSeedInfoArr []constructdbg.SeedInfo, buf []MapingKmerInfo, strand bool, eID uint32) []MapingKmerInfo {
	buf = buf[:0]
	i, j := 0, 0
...
func GetEdgeMaxPathInfo(emi EdgeMapInfo, readSeedInfoArr []constructdbg.SeedInfo, direction uint8, readLen, startPos int, edgesArr []constructdbg.DBGEdge, buf []MapingKmerInfo, MinChainScoreIdentityPercent float32) *MaxPathInfo {
	eID := emi.EdgeID
	e := &edgesArr[eID]
	el := e.GetSeqLen()
	var start, end int
	...
	sort.Sort(constructdbg.SeedInfoArr(sIA))
	...
	edgeRegLen := endE - startE
	min := utils.MinInt(readRegLen, edgeRegLen)
	if float32(mpi.Score) > float32(el)*MinChainScoreIdentityPercent && utils.AbsInt(readRegLen-edgeRegLen) < min/10 && min > el/2 {
		mpi.EID = eID
	} else {
		return nil
	}
...
func GetMergedEdgesMaxPathInfo(emiArr []EdgeMapInfo, readSeedInfoArr []constructdbg.SeedInfo, direction uint8, readLen, startPos int, edgesArr []constructdbg.DBGEdge, nodesArr []constructdbg.DBGNode, buf []MapingKmerInfo, MinChainScoreIdentityPercent float32, SeedLen, WinSize, kmerlen int, logfp io.Writer) MaxPathInfo {
	var mpi MaxPathInfo
	eID := emiArr[0].EdgeID
	var e constructdbg.DBGEdge
	if len(emiArr) == 1 {
		e = edgesArr[eID]
		if emiArr[0].Strand == constructdbg.MINUS {
			e.Utg.Ks = constructdbg.GetReverseCompByteArr(e.Utg.Ks)
		}
	} else {
		...
		e = constructdbg.ConcatEdgePathUtg(path, nID, edgesArr, nodesArr, kmerlen)
		if e.GetSeqLen() == 0 {
			return mpi
		}
		if direction == constructdbg.BACKWARD {
			constructdbg.ReverseCompByteArr(e.Utg.Ks)
		}
	}
	...
	bufSize := WinSize * 10
	kaBuf := make([]KmerInfo, bufSize)
	ka := make([]KmerInfo, 50)
	//rs := make([]byte, 500)
	ka = GetSeqMiniKmerInfoArr(e.Utg.Ks, kaBuf, ka, uint32(e.ID), SeedLen, WinSize)
	sort.Sort(KIArr(ka))
	e.SeedInfoArr = make([]constructdbg.SeedInfo, len(ka))
	for x, ki := range ka {
		e.SeedInfoArr[x] = constructdbg.SeedInfo(ki.Kmer)
	...
	buf = GetEdgeInterSectionKmerInfo(e.SeedInfoArr, sIA, buf, constructdbg.PLUS, eID)
	...
	// ka need sort edge position
	sc, a, b := GetSumMappingKIArrLen(kb, constructdbg.PLUS, el)
	...
	bsA = InitStrandInfo(kb, bsA, constructdbg.PLUS)
	mpi = GetMaxScoreArr(kb, constructdbg.PLUS, SeedLen, 1, 100, 1, el, bsA, true)
	if mpi.LenArr == 0 {
		var nm MaxPathInfo
		return nm
	}
...
func IsEuqlEdgeFreqArr(path1, path2 []constructdbg.EdgeFreq, rev bool) bool {
...
func Transform2EdgeFreq(emiPath []EdgeMapInfo) []constructdbg.EdgeFreq {
	path := make([]constructdbg.EdgeFreq, len(emiPath))
	for i, emi := range emiPath {
		path[i].ID = constructdbg.DBG_MAX_INT(emi.EdgeID)
	}
	return path
}

func Transform2EdgeMapInfo(path []constructdbg.EdgeFreq, edgesArr []constructdbg.DBGEdge, e *constructdbg.DBGEdge, strand bool, direction uint8) []EdgeMapInfo {
	emiArr := make([]EdgeMapInfo, len(path))
	for i, ef := range path {
		emiArr[i].EdgeID = uint32(ef.ID)
		ne := &edgesArr[ef.ID]
		emiArr[i].EdgeLen = uint32(ne.GetSeqLen())
		emiArr[i].Strand = constructdbg.GetNextEdgeStrand2(e, ne, strand, direction)
		e = ne
	}
	return emiArr
}

func GetNextNGSPath(NGSPathMat [][]constructdbg.EdgeFreq, emiPath []EdgeMapInfo, e *constructdbg.DBGEdge, edgesArr []constructdbg.DBGEdge, strand bool, direction uint8) [][]EdgeMapInfo {
	path := Transform2EdgeFreq(emiPath)
	var emiMat [][]EdgeMapInfo
	for _, p := range NGSPathMat {
		idx := constructdbg.IndexEdgeFreqArr(p, e.ID)
		if idx == len(p)-1 {
			p = constructdbg.GetReverseEdgeFreqArr2(p)
			idx = 0
		}
		id := p[idx+1].ID
		f1 := constructdbg.IsInComing(e.EdgeIDIncoming, id)
		f2 := constructdbg.IsInComing(e.EdgeIDOutcoming, id)
		if !f1 && !f2 {
		...
		if (direction == constructdbg.FORWARD && strand) || (direction == constructdbg.BACKWARD && !strand) {
			if f2 {
				min := utils.MinInt(idx+1, len(path))
				...
				min := utils.MinInt(len(p)-idx, len(path))
	...
	for i := range emiMat {
		...
		for j, emi := range emiMat[i] {
			...
			strand = constructdbg.GetNextEdgeStrand2(lastE, ne, strand, direction)
...
func GetNextEMIArr(edgesArr []constructdbg.DBGEdge, lastEMI EdgeMapInfo, direction uint8, maxPathInfoArr []MaxPathInfo, mpiIdx, distance uint16, kmerlen int, logfp io.Writer) (emiArr []EdgeMapInfo) {
	e := &edgesArr[lastEMI.EdgeID]
	strand := lastEMI.Strand
	var ecoming [bnt.BaseTypeNum]constructdbg.DBG_MAX_INT
	if direction == constructdbg.FORWARD {
		if strand {
	...
	for _, id := range ecoming {
		...
		emi.EdgeLen = uint32(ne.GetSeqLen())
		emi.Strand = constructdbg.GetNextEdgeStrand2(e, ne, strand, direction)
...
func GetNextEMIMat(edgesArr []constructdbg.DBGEdge, lastEMI EdgeMapInfo, direction uint8, MaxPathLen uint32, kmerlen int, maxPathInfoArr []MaxPathInfo, mpiIdx, distance uint16, logfp io.Writer) (emiMat [][]EdgeMapInfo) {
...
func GetAllPathArr(maxPathInfoArr []MaxPathInfo, direction uint8, mpiIdx, distance int, nea []EdgeMapInfo, edgesArr []constructdbg.DBGEdge) (emiMat [][]EdgeMapInfo) {

	return
}

func GetEdgeType(e *constructdbg.DBGEdge) string {
	if e.GetUniqueFlag() > 0 {
		return "Unique"
...
func GetMaxScorePath(emiMat [][]EdgeMapInfo, edgesArr []constructdbg.DBGEdge, direction uint8, kmerlen int) int {
	idx := -1
				...
				if sia[i].Score == 0 {
					...
					if direction == constructdbg.FORWARD {
						if start < sia[i].End && end > sia[i].End {
							...
							min := utils.MinInt(distE, distR)
							gapLen := utils.AbsInt(distE - distR)
...
func CheckNGSEMIMat(pm [][]EdgeMapInfo, emiArr []EdgeMapInfo, edgesArr []constructdbg.DBGEdge, maxPathInfoArr []MaxPathInfo, direction uint8, mpiIdx, distance uint16, kmerlen int) [][]EdgeMapInfo {
	pmIdx := 0
	for _, arr := range pm {
...
func DeleteContainedPath(pm [][]EdgeMapInfo) [][]EdgeMapInfo {
	...
	for _, arr := range pm {
		var ok bool
		for j := 0; j < idx; j++ {
			min := utils.MinInt(len(arr), len(pm[j]))
...
func TransformPathMat2String(pathMat [][][]EdgeMapInfo, maxPathInfoArr []MaxPathInfo, rID int, rl int, anotition string) string {
	...
	ps = append(ps, '\t')
	if emi.Strand == constructdbg.PLUS {
		ps = append(ps, '+')
	} else {
		ps = append(ps, '-')
	}
...
func FindMappingPath(readSeedInfoArr []constructdbg.SeedInfo, maxPathInfoArr []MaxPathInfo, anchorIdx int, priorIdxArr []uint16, edgesArr []constructdbg.DBGEdge, nodesArr []constructdbg.DBGNode, kmerlen, readLen int, buf []MapingKmerInfo, MinChainScoreIdentityPercent float32, WinSize int, logfp io.Writer) [][][][]EdgeMapInfo {
	...
	for anchorIdx >= 0 {
		...
		// sequence read BACKWARD
		{
			...
			distance := GetDistance(startE, endE, edgesArr[mpi.EID].GetSeqLen(), strand, constructdbg.BACKWARD)
			...
			for {
				...
				strand = lastEMI.Strand
				emiArr := GetNextEMIArr(edgesArr, lastEMI, constructdbg.BACKWARD, maxPathInfoArr, mpiIdx, distance, kmerlen, logfp)
				if len(emiArr) == 0 {
				...
				npa := e.NGSPathArr
				if len(npa) > 0 {
					pm = GetNextNGSPath(npa, path, e, edgesArr, strand, constructdbg.BACKWARD)
					pm = DeleteContainedPath(pm)
					// check pm by mpi
					pm = CheckNGSEMIMat(pm, emiArr, edgesArr, maxPathInfoArr, constructdbg.BACKWARD, mpiIdx, distance, kmerlen)
					if DebugModel {
					...
					if len(emiArr) == 1 && len(pm) == 1 {
						...
						mpiIdx, distance = GetBoundaryMPIDistance(maxPathInfoArr, constructdbg.BACKWARD, mpiIdx, distance, kmerlen, pm[0])
						continue
					}
				}
				...
				if len(emiArr) == 1 {
					...
					mpiIdx, distance = GetBoundaryMPIDistance(maxPathInfoArr, constructdbg.BACKWARD, mpiIdx, distance, kmerlen, emiArr)
					continue
				}
				...
				if len(pm) > 0 {
					for _, emi := range emiArr {
						if !IsInEMIMat(pm, emi.EdgeID) {
							...
							arr = append(arr, emi)
							//ne := &edgesArr[emi.EdgeID]
							ea := GetNextEMIArr(edgesArr, emi, constructdbg.BACKWARD, maxPathInfoArr, mpiIdx, distance, kmerlen, logfp)
							if len(ea) == 1 {
								arr = append(arr, ea[0])
							}
							pm = append(pm, arr)
					...
					pm = GetNextEMIMat(edgesArr, lastEMI, constructdbg.BACKWARD, MaxPathLen, kmerlen, maxPathInfoArr, mpiIdx, distance, logfp)
				}
				...
				if len(pm) == 0 {
					...
					if IsBubble(e1, e2, nodesArr) {
						if len(path) > 0 {
							...
							var np []EdgeMapInfo
							path = np
							ta := GetNextEMIArr(edgesArr, pm[0][0], constructdbg.BACKWARD, maxPathInfoArr, mpiIdx, distance, kmerlen, logfp)
							if len(ta) == 1 {
						...
						mpiIdx, distance = GetBoundaryMPIDistance(maxPathInfoArr, constructdbg.BACKWARD, mpiIdx, distance, kmerlen, []EdgeMapInfo{pm[0][0], path[0]})
						continue
					}
				}
				...
				// get mpi
				{
					...
					for i, emiArr := range pm {
						mpiArr[i] = GetMergedEdgesMaxPathInfo(emiArr, readSeedInfoArr, constructdbg.BACKWARD, readLen, rStart-int(distance)*11/10+kmerlen, edgesArr, nodesArr, buf, MinChainScoreIdentityPercent, SeedLen, WinSize, kmerlen, logfp)
					...
					if len(mpiArr) == 1 {
						...
						mpiIdx, distance = GetBoundaryMPIDistance(maxPathInfoArr, constructdbg.BACKWARD, mpiIdx, distance, kmerlen, pm[idx])
						continue
					} else {
				...
				//idx := GetMaxScorePath(pm, edgesArr, constructdbg.FORWARD, kmerlen)
			}
		...
		// sequence read FORWARD
		{
			...
			_, _, strand, startE, endE := GetMPIInfo(emi.MPI)
			mpiIdx := uint16(anchorIdx)
			distance := GetDistance(startE, endE, edgesArr[emi.EdgeID].GetSeqLen(), strand, constructdbg.FORWARD)
			for {
				lastEMI := path[len(path)-1]
				CheckCyclePath(path, kmerlen)
				e := &edgesArr[lastEMI.EdgeID]
				...
				strand = lastEMI.Strand
				emiArr := GetNextEMIArr(edgesArr, lastEMI, constructdbg.FORWARD, maxPathInfoArr, mpiIdx, distance, kmerlen, logfp)
				if len(emiArr) == 0 {
					fmt.Fprintf(logfp, "[FindMappingPath]Next emiArr len:%d\n", len(emiArr))
					//fmt.Printf("[FindMappingPath]Incoming:%v Outcoming:%v startNd:%d endNd:%d\n", e.EdgeIDIncoming, e.EdgeIDOutcoming, nodesArr[e.StartNID], nodesArr[e.EndNID])
					break
				...
				npa := e.NGSPathArr
				if len(npa) > 0 {
					pm = GetNextNGSPath(npa, path, e, edgesArr, strand, constructdbg.FORWARD)
					pm = DeleteContainedPath(pm)
					// check pm by mpi
					pm = CheckNGSEMIMat(pm, emiArr, edgesArr, maxPathInfoArr, constructdbg.FORWARD, mpiIdx, distance, kmerlen)
					if DebugModel {
						fmt.Fprintf(logfp, "[FindMappingPath]NGSPath:%v\n\tpm:%v\n", npa, pm)
					}
					if len(emiArr) == 1 && len(pm) == 1 {
						//ea := Transform2EdgeMapInfo(pm[0], edgesArr, e, strand)
						path = append(path, pm[0]...)
						mpiIdx, distance = GetBoundaryMPIDistance(maxPathInfoArr, constructdbg.FORWARD, mpiIdx, distance, kmerlen, pm[0])
						continue
					}
				}
				...
				if len(emiArr) == 1 {
					path = append(path, emiArr[0])
					mpiIdx, distance = GetBoundaryMPIDistance(maxPathInfoArr, constructdbg.FORWARD, mpiIdx, distance, kmerlen, emiArr)
					continue
				}
				...
				if len(pm) > 0 {
					for _, emi := range emiArr {
						if !IsInEMIMat(pm, emi.EdgeID) {
							...
							arr = append(arr, emi)
							//ne := &edgesArr[emi.EdgeID]
							ea := GetNextEMIArr(edgesArr, emi, constructdbg.FORWARD, maxPathInfoArr, mpiIdx, distance, kmerlen, logfp)
							if len(ea) == 1 {
								arr = append(arr, ea[0])
							}
							pm = append(pm, arr)
					...
					pm = GetNextEMIMat(edgesArr, lastEMI, constructdbg.FORWARD, MaxPathLen, kmerlen, maxPathInfoArr, mpiIdx, distance, logfp)
				}
				...
				if len(pm) == 0 {
					...
					if IsBubble(e1, e2, nodesArr) {
						if len(path) > 0 {
							...
							var np []EdgeMapInfo
							path = np
							ta := GetNextEMIArr(edgesArr, pm[0][0], constructdbg.FORWARD, maxPathInfoArr, mpiIdx, distance, kmerlen, logfp)
							if len(ta) == 1 {
								path = append(path, ta[0])
							} else {
								fmt.Fprintf(logfp, "[FindMappingPath]ta:%v\n", ta)
						...
						mpiIdx, distance = GetBoundaryMPIDistance(maxPathInfoArr, constructdbg.FORWARD, mpiIdx, distance, kmerlen, []EdgeMapInfo{pm[0][0], path[0]})
						continue
					}
				}
				// check  self cycle path
				...
				// get mpi
				{
					...
					for i, emiArr := range pm {
						mpiArr[i] = GetMergedEdgesMaxPathInfo(emiArr, readSeedInfoArr, constructdbg.FORWARD, readLen, rEnd+int(distance)*9/10-kmerlen, edgesArr, nodesArr, buf, MinChainScoreIdentityPercent, SeedLen, WinSize, kmerlen, logfp)
						pm[i][0].Distance = uint16(mpiArr[i].Score)
					}
					if DebugModel {
						for i := range mpiArr {
					...
					if len(mpiArr) == 1 {
						idx := IndexMaxPathInfoArr(pm, &mpiArr[0])
						path = append(path, pm[idx]...)
						mpiIdx, distance = GetBoundaryMPIDistance(maxPathInfoArr, constructdbg.FORWARD, mpiIdx, distance, kmerlen, pm[idx])
						continue
					} else {
						fmt.Fprintf(logfp, "[FindMappingPath]len(mpiArr):%d\n", len(mpiArr))
						break
...
func MapingONTReadsChunk(dbgKmerInfoPac DBGKmerInfoPac, cs <-chan ReadsBucketInfo, SeedLen, winSize, GapCost, MaxGapLen, kmerlen int, wc chan<- string, edgesArr []constructdbg.DBGEdge, nodesArr []constructdbg.DBGNode, SeqType uint8, MaxPathLen, MaxBubbleSeqLen int, MinChainScoreIdentityPercent float32, prefix string, processID int) {
...
func PrintDBGEdges(edgesArr []constructdbg.DBGEdge, prefix string) {
...
func DeconstructDBG(c cli.Command) {
	...
	opt := Options{gOpt, 0, 0, 0, 0, 0, 0, 0, 0, 0, "", false, true, false}
	tmp, suc := checkArgs(c)
	if suc == false {
		log.Fatalf("[DeconstructDBG] check Arguments error, opt: %v\n", tmp)
	}
	DebugModel = tmp.Debug
	BuckReadsNum = 100
	EdgeMapKmerMin = 40
	//opt.TipMaxLen = tmp.TipMaxLen
	opt.MinDepth = tmp.MinDepth
	opt.AvgDepth = tmp.AvgDepth
	if tmp.MinUniqueEdgeLen < tmp.Kmer {
		tmp.MinUniqueEdgeLen = tmp.Kmer
	}
	...
	opt.MinChainScoreIdentityPercent = tmp.MinChainScoreIdentityPercent
	opt.ONTFn = tmp.ONTFn
	opt.Correct = tmp.Correct
	opt.Haplotype = tmp.Haplotype
	GapCost := 1
	MaxGapLen := 2000
	SeqType := uint8(1)
	MaxPathLen := 10
	...
	fc := make(chan int, 1)
	go constructdbg.NodesArrReader(nodesfn, nodesArr, opt.Kmer, fc)
	...
	edgesArr := constructdbg.ReadEdgesFromFile(edgesfn, constructdbg.DBG_MAX_INT(eSize))
	<-fc

	go constructdbg.CheckInterConnectivity(edgesArr, nodesArr)
	//constructdbg.PrintTmpDBG(nodesArr, edgesArr, opt.Prefix)

	uniqueNum, bubbleRepeatNum, twoCycleNum, selfCycle, selfCycleSameComingNum, bubbleEdgeNum := constructdbg.SetDBGEdgesUniqueFlag(edgesArr, nodesArr, opt.Kmer)
	constructdbg.AddNodeInfo2DBGEdgeArr(edgesArr, nodesArr)
	runtime.GC()
	...
	if _, err := os.Stat(pathFn); err != nil {
		...
		minimerSize := constructdbg.GetCuckoofilterDBGSampleSize(edgesArr, int64(opt.WinSize), int64(opt.Kmer))
	...
	t1 := time.Now()
	{
		cs := make(chan constructdbg.MapingInfo, 1000)
		go constructdbg.LoadPathFromFile(pathFn, cs)
