...
const (
	// ErrorRate default error rate for long Reads
	ErrorRate = 0.15
	//MinKmerMapLen note min kmer map len
	MinKmerMapLen = 60
)
...
func GetRawReads(rc chan []Seq, cfgInfo CfgInfo, blockSize int) {
	var slice []Seq
	ID := 0
	num := 0
...
func GetNextEArr(eID uint32, node DBGNode) (eIDArr []uint32) {
	var direction uint8
...
func FindMEKArr(NoContainMKArr []MapEdgeKmers, eIDArr []uint32, qPos int32) (proArr []MapEdgeKmers) {
...
func ExtendSeedEdge(seedEdgeIndex int, NoContainMKArr []MapEdgeKmers, edgesArr []DBGEdge, nodesArr []DBGNode, mks []MergeKmerInfo, seed int, ReadLen int32, readSeq []byte) (readPath []uint32) {
	// extend left flank
	{
		smk := NoContainMKArr[seedEdgeIndex]
		readPath = append(readPath, uint32(smk.EID))
		var nID uint32
		var qPos int32
		direction := BACKWARD
		if smk.Strand == 0 {
		...
		for nID > 0 && qPos > 0 {
			...
			if len(proArr) == 0 {
				...
				readPath = append(readPath, uint32(proArr[0].EID))
				qPos = proArr[0].Start
				...
				if proArr[1].End == proArr[1].End {
					...
					if matchBase[1] > matchBase[0] {
						readPath = append(readPath, uint32(proArr[1].EID))
						qPos = proArr[1].Start
	...
	readPath = ReverseDbgMaxIntArr(readPath)
...
func paraFoundPath(rc chan []MergeKmerInfo, wc chan []uint32, edgesArr []DBGEdge, nodesArr []DBGNode, seed int32, seqSlice []Seq) {
...
func WriteReadPath(wc chan []uint32, numCPU int) {
	terminalNum := 0
	for {
		rp := <-wc
		if len(rp) == 0 {
...
func MapDBG(c cli.Command) {
	...
	cfgInfo, err := ParseCfg(string(fnName), false, false)
	if err != nil {
