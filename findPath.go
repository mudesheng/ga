...
func findNextEdge(node DBGNode, eID uint32, rArr []sam.Record) (neID uint32, recd sam.Record) {
	...
	for _, v := range rArr {
		...
		refIDArr = append(refIDArr, uint32(id))
	}
...
func isInEdgesArr(arr []uint32, e uint32) bool {
	for _, v := range arr {
		if v == e {
			return true
		}
...
func IsConnectEdges(miArr []MapInfo, edgesArr []DBGEdge, nodesArr []DBGNode) bool {
	if len(miArr) < 2 {
		return true
	}
...
func paraFindShortMappingPath(rc chan [2]FastMapRecord, wc chan []uint32, edgesArr []DBGEdge, nodesArr []DBGNode) {
	// totalM, totalI, totalD := 0, 0, 0
	for {
		rArr := <-rc
		if len(rArr[0].Minfo) == 0 {
			var guardPath []uint32
			wc <- guardPath
			break
		}
...
func IsCyclePath(path []uint32) bool {
	cycle := false
...
func IsNodeCyclePath(path []uint32, edgesArr []DBGEdge) bool {
	// node array
	var narr []uint32
...
func WriteShortPathToDBG(wc chan []uint32, edgesArr []DBGEdge, numCPU int) {
	terminalNum := 0
...
/*func GetNextEdgeIDArr(e *DBGEdge, nodesArr []DBGNode, strand bool, direction uint8) (eIDArr []uint32) {
	if direction == FORWARD {
		if strand == PLUS {
			if e.EndNID == 0 {
				return
...
/*func GetNextEIDArr(eID uint32, node DBGNode) (neIDArr []uint32) {
	var direction uint8
	for i := 0; i < bnt.BaseTypeNum; i++ {
		if node.EdgeIDIncoming[i] == eID {
			direction = BACKWARD
...
func GetNextEID(eID uint32, node DBGNode) (neID uint32) {
	var direction uint8
	for i := 0; i < bnt.BaseTypeNum; i++ {
		if node.EdgeIDIncoming[i] == eID {
			direction = BACKWARD
...
func IsDirectionUniqueEdge(edge DBGEdge, node DBGNode) bool {
	var direction uint8
	in, out := 0, 0
	for i := 0; i < bnt.BaseTypeNum; i++ {
		if node.EdgeIDIncoming[i] > 0 {
...
func MergePath(pathMat []Path, beID uint32) (uniquePath Path, num int) {
...
func IndexLastEID(arr []uint32, eID uint32) int {
...
func IndexEIDAllowDiffLen(arr []uint32, eID uint32, idx, diffLen int) int {
...
func IndexUniqueEdge(consisPathArr []uint32, edgesArr []DBGEdge, direction uint8) int {
	if direction == FORWARD {
...
func mutualReachable(a1, a2 []uint32, eID1, eID2 uint32) bool {
	idx1 := IndexEID(a1, eID1)
	idx2 := IndexEID(a1, eID2)
...
func FindConsistenceAndMergePath(a1, a2 []uint32, eID1, eID2 uint32) (mergeArr []uint32, consisFlag bool) {
	idx1 := IndexEID(a1, eID1)
	idx2 := IndexEID(a1, eID2)
	...
	if idx1 < 0 || idx2 < 0 || idx3 < 0 || idx4 < 0 {
		log.Fatalf("[FindConsistenceAndMergePath] eID not found in the PathArr, eID1: %v or eID2: %v\n\ta1: %v\n\ta2:%v\n", eID1, eID2, a1, a2)
	}

	if idx1 > idx2 {
		log.Fatalf("[FindConsistenceAndMergePath] idx1: %v > idx2: %v in the a1: %v\n", idx1, idx2, a1)
	}
	if idx3 > idx4 {
		log.Fatalf("[FindConsistenceAndMergePath] idx3: %v > idx4: %v in the a2: %v\n", idx3, idx4, a2)
	}
	...
	mergeArr = append(mergeArr, a1[idx2+1:i]...)
	if i < len(a1) {
		mergeArr = append(mergeArr, a1[i:]...)
	} else if j < len(a2) {
		mergeArr = append(mergeArr, a2[j:]...)
	}

	return mergeArr, consisFlag
}

func GetDirection(node DBGNode, eID uint32) (direction uint8) {
	if eID == 0 {
...
func connectNodeID(e1, e2 DBGEdge) uint32 {
	if e1.StartNID == e2.StartNID || e1.StartNID == e2.EndNID {
		return e1.StartNID
	} else if e1.EndNID == e2.StartNID || e1.EndNID == e2.EndNID {
		return e1.EndNID
...
/*func ResetProcessFlag(edgesArr []DBGEdge) {
	for i, _ := range edgesArr {
		edgesArr[i].ResetProcessFlag()
	}
}*/

func DeleteEdgeID(nodesArr []DBGNode, nID, eID uint32) (success bool) {
	for i := 0; i < bnt.BaseTypeNum; i++ {
...
func CheckConsistenceArr(arr1, arr2 []uint32, eID uint32) (mp []uint32, ok bool) {
...
func FoundAllPath(edge DBGEdge, eIDArr []uint32) (path Path, num int) {
	for _, eID := range eIDArr {
		var mergePath Path
		for _, p := range edge.PathMat {
			if IsInuint32Arr(p.IDArr, eID) {
...
func GetUniqueOne(src, dst []uint32) (unique uint32) {
	for _, sID := range src {
		for _, dID := range dst {
			if sID == dID {
				unique = sID
...
/*func FindMaxUnqiuePath(edgesArr []DBGEdge, nodesArr []DBGNode) [][]uint32 {

consistenceA := make([][]uint32, len(edgesArr))
...
for _, e := range edgesArr {
		...
		if IsInuint32Arr(right, reID) == false {
			...
			for node.ID > 0 {
				...
				if len(eIDArr) == 0 {
					...
					if IsInuint32Arr(right, eIDArr[0]) {
						break
					}
					...
					for i := len(right) - 2; i >= 0 && backLen < MAX_INSERT_SIZE; i-- {
						...
						if num == 1 {
							if path.Freq >= MIN_ALLOW_PATH_FREQ {
								...
								if IsInuint32Arr(right, uniqueID) {
									break
								}
	...
	if e.StartNID > 0 && IsDirectionUniqueEdge(e, nodesArr[e.StartNID]) {
		...
		if IsInuint32Arr(left, leID) == false {
			...
			for node.ID > 0 {
				...
				if len(eIDArr) == 0 {
					...
					if IsInuint32Arr(left, eIDArr[0]) {
						break
					}
					...
					for i := len(left) - 2; i >= 0 && backLen < MAX_INSERT_SIZE; i-- {
						...
						if num == 1 {
							if path.Freq >= MIN_ALLOW_PATH_FREQ {
								...
								if IsInuint32Arr(left, uniqueID) {
									break
								}
	...
	for stk.Len() > 0 {
		...
		for _, path := range ne.PathMat {
			...
			if MIN_PATH_LEN-1 <= j && j < len(path.IDArr)-1 {
				slc := GetReverseuint32Arr(path.IDArr[:j+1])
		...
		for j := 0; j < bnt.BaseTypeNum; j++ {
			if direction == FORWARD && node.EdgeIDIncoming[j] > 0 {
				var npci PathCrossInfo
				npci.EdgeID = node.EdgeIDIncoming[j]
				npci.Node = node
				npci.RemainLen = pci.RemainLen
				stk.PushBack(npci)
			} else if direction == BACKWARD && node.EdgeIDOutcoming[j] > 0 {
				var npci PathCrossInfo
...
consistenceA := make([][]uint32, len(edgesArr))
{
	for _, e := range edgesArr {
		...
		if bnum == 1 {
			tp := GetReverseuint32Arr(buniquePath.IDArr)
...
for i, e := range edgesArr {
	...
	if j >= MIN_PATH_LEN-1 {
		...
		for x := extendPathLen - 1; x >= 0; x-- {
			...
			if y >= 0 && z >= 0 {
				if y >= z {
					consistenceA[ne.ID] = GetReverseuint32Arr(consistenceA[ne.ID])
				}
...
ResetProcessFlag(edgesArr)
{
	for i, _ := range edgesArr {
		...
		x := IndexUniqueEdge(consistenceA[i], edgesArr, FORWARD)
		y := IndexUniqueEdge(consistenceA[i], edgesArr, BACKWARD)
		if x < y && y-x+1 >= MIN_PATH_LEN {
			...
			for j := x + 1; j <= y; j++ {
				...
				if GetDirection(nodesArr[nID], e2.ID) == FORWARD {
					if e1.StartNID == nID {
						RCEdge(edgesArr, e1.ID)
					}
					if e2.EndNID == nID {
						RCEdge(edgesArr, e2.ID)
					}
					e1 = edgesArr[e1.ID]
					e2 = edgesArr[e2.ID]
					DeleteEdgeID(nodesArr, e1.EndNID, e1.ID)
					ConcatEdges(edgesArr, e1.ID, e2.ID, e1.ID)
					...
					ConcatEdges(edgesArr, e2.ID, e1.ID, e1.ID)
					if e2.GetUniqueFlag() > 0 {
						edgesArr[e2.ID].SetDeleteFlag()
...
func GetNeighbourEID(eID uint32, nID uint32, edgesArr []DBGEdge, nodesArr []DBGNode, max_insert_size int) (neighbourEIDArr []uint32) {
	...
	for len(stk) > 0 {
		...
		for _, eID := range eIDArr {
			if IsInuint32Arr(neighbourEIDArr, eID) {
				continue
			}
...
func IsIntersection(rightEIDArr, leftEIDArr []uint32) bool {
	for _, eID := range rightEIDArr {
		if IsInuint32Arr(leftEIDArr, eID) {
			return true
		}
	}
	return false
...
func IsContained(subArr, allArr []uint32) bool {
	for _, id := range subArr {
		if IsInuint32Arr(allArr, id) == false {
			return false
		}
	}
...
/*func IsInPathMat(pathMat []Path, eID uint32) bool {
	for _, p := range pathMat {
		if IsInuint32Arr(p.IDArr, eID) {
			return true
		}
...
func IsParaEdge(edge DBGEdge, nodesArr []DBGNode, pathMat []Path) bool {
	for i := 0; i < 2; i++ {
		var nID uint32
		if i == 0 {
			nID = edge.StartNID
...
func GetInterNodeID(edgeSrc, edgeDst DBGEdge) uint32 {
...
func GetMergePathArr(EpathMat []Path, edgesArr []DBGEdge, nodesArr []DBGNode) (mergePathArr []Path) {
	var pathMat []Path
	for _, pi := range EpathMat {
		var p Path
		p.IDArr = make([]uint32, len(pi.IDArr))
	...
	for i := 0; i < len(pathMat); i++ {
		...
		// check unique path
		var np Path
		np.IDArr = make([]uint32, len(p.IDArr))
...
func FindMaxUnqiuePath(edgesArr []DBGEdge, nodesArr []DBGNode) [][2][]uint32 {

	consistenceA := make([][2][]uint32, len(edgesArr))
		...
		if j >= MIN_PATH_LEN-1 {
			...
			for x := extendPathLen - 1; x >= 0; x-- {
				...
				if y >= 0 && z >= 0 {
					...
					consistenceMergePath, _ := FindConsistenceAndMergePath(consistenceA[i], consistenceA[ne.ID], se.ID, ne.ID)
					if len(consistenceMergePath) >= len(consistenceA[i]) {
						if IsCyclePath(consistenceMergePath) {
							break
						}
...
func IsComingInNode(node DBGNode, eID uint32) bool {
	for i := 0; i < bnt.BaseTypeNum; i++ {
		if node.EdgeIDIncoming[i] == eID {
			return true
		}
...
func CleanDBG(edgesArr []DBGEdge, nodesArr []DBGNode) {
	deleteNum := 0
	for i, e := range edgesArr {
...
func Convert2LA(fields []string, RefIDMapArr []uint32) (la LA) {
...
func InsertIDArr(RefIDArr []uint32, ID uint32) []uint32 {
...
func GetLARecord(lafn string, RefIDMapArr []uint32, lac chan []LA, numCPU int) {
	fp, err := os.Open(lafn)
	if err != nil {
...
func MergeLA(laArr []LA, gapRegArr []GapRegion, initLa LA) (la LA) {
	...
	for {
		left := findNearLA(laArr, initLa, BACKWARD)
	...
	for {
		right := findNearLA(laArr, initLa, FORWARD)
...
func GetLARegion(laArr []LA, eID uint32) []LA {
	b, e := -1, -1
...
func findEdgeLA(eID uint32, laArr []LA, gapRegArr []GapRegion, QuyPos int, QuyDirection uint8) (la LA) {
...
func findNextProEdge(eIDArr []uint32, laArr []LA, gapRegArr []GapRegion, QuyPos int, QuyDirection uint8) (nLaArr []LA) {
		...
		if len(nLaArr) == 0 {
			...
			if proLa.AlgnLen == nLaArr[0].AlgnLen {
				...
				if !choosed {
					...
					proLenP := float64(proLa.RefE-proLa.RefB) / float64(proLa.RefLen-2*len(AdpaterSeq))
					nlaLenP := float64(nLaArr[0].RefE-nLaArr[0].RefB) / float64(nLaArr[0].RefLen-2*len(AdpaterSeq))
...
func findNextProPath(subp [2][]uint32, laArr []LA, gapRegArr []GapRegion, QuyPos int, QuyDirection uint8) int {
	idx := -1
...
func SumLen(eIDArr []uint32, edgesArr []DBGEdge) (sl int) {
	sl = Kmerlen - 1
	for _, eID := range eIDArr {
...
func IsSubPath(srcArr, subArr []uint32) (is bool) {
...
func GetNextShortPath(consistenceA [][]uint32, i, j int) (ni, nj int) {
	idx := IndexEID(consistenceA[i], uint32(i))
	if idx < j {
...
func GetNextDirection(eID uint32, edge DBGEdge, nodesArr []DBGNode) (direction uint8) {
	if edge.StartNID > 0 {
		n1 := nodesArr[edge.StartNID]
		if GetDirection(n1, edge.ID) == FORWARD {
	...
	if edge.EndNID > 0 {
		...
		if GetDirection(n2, edge.ID) == FORWARD {
...
func GetNextConsisPath(edgesArr []DBGEdge, nodesArr []DBGNode, left []uint32, consistenceA [][]uint32) (n, m int) {
	for i := len(left) - 1; i > 0; i-- {
		if len(consistenceA[left[i]]) > 0 {
			forA := consistenceA[left[i]]
			direction := GetNextDirection(left[i-1], edgesArr[left[i]], nodesArr)
			if direction == FORWARD {
				forA = GetReverseuint32Arr(forA)
			}
...
func GetNextConsistenceAPosition(consistenceA [][]uint32, eID uint32, direction uint8) (n, m int) {
...
func IsInLAArr(laArr []LA, eID uint32) bool {
	for _, la := range laArr {
		if la.RefID == eID {
			return true
		}
...
func GetRemainArr(csArr []uint32, eID, lasteID uint32) (remainArr []uint32) {
	idx := IndexEID(csArr, eID)
	if 0 < idx && idx < len(csArr)-1 {
		if lasteID == csArr[idx-1] {
			if len(csArr)-(idx+1) > 0 {
				remainArr = make([]uint32, len(csArr)-(idx+1))
				copy(remainArr, csArr[idx+1:])
			}
		} else if lasteID == csArr[idx+1] {
			remainArr = GetReverseuint32Arr(csArr[0:idx])
		} else {
		...
		if idx == 0 {
			if lasteID != csArr[idx+1] {
				remainArr = make([]uint32, len(csArr)-(1))
				copy(remainArr, csArr[1:])
			}
		} else { // idx == len(csArr)-1
			if lasteID != csArr[len(csArr)-2] {
				remainArr = GetReverseuint32Arr(csArr[0 : len(csArr)-1])
			}
		}
	}
...
/*func GetNeedSortEID(edgesArr []DBGEdge, rc chan uint32, numCPU int) {
	for _, e := range edgesArr {
...
/*func paraSortPathMat(edgesArr []DBGEdge, rc chan uint32) {
...
func SortPathMat(edgesArr []DBGEdge) {
...
func GetNextPathLA(eIDArr []uint32, laArr []LA) (remainLaArr []LA) {
	for _, eID := range eIDArr {
		edgeLaArr := GetLARegion(laArr, eID)
		if len(edgeLaArr) > 0 {
...
/*func findConsistenceExtend(pathMat []Path, edgesArr []DBGEdge, nodesArr []DBGNode) (consis []PathFreqArr) {
...
/*func GetInterSection(arr1, arr2 []uint32) (interSecArr []uint32) {
	for _, e1 := range arr1 {
		if IsInuint32Arr(arr2, e1) {
...
func CheckAndCutLocalCycle(eIDArr []uint32) (cutArr []uint32, cuted bool) {
	var cycleArr []uint32
	cutArr = eIDArr
	i := len(eIDArr) - 2
	for i >= 0 {
...
func ExtendGap(la LA, gapRegArr []GapRegion) LA {
	extLa := la
	if la.RefB > len(AdpaterSeq)+Kmerlen/REF_OVL_FACTOR {
		var gp GapRegion
		if la.Minus {
			gp.Begin = la.QuyE
			gp.End = la.QuyE + la.RefB - len(AdpaterSeq)
		} else {
			gp.Begin = la.QuyB - (la.RefB - len(AdpaterSeq))
			gp.End = la.QuyB
		}

		if IsCrossGapRegArr(gapRegArr, gp) {
			extLa.RefB = len(AdpaterSeq)
		}
	}
	if la.RefE < la.RefLen-len(AdpaterSeq)-Kmerlen/REF_OVL_FACTOR {
		var gp GapRegion
		if la.Minus {
			gp.Begin = la.QuyB - (la.RefLen - la.RefE - len(AdpaterSeq))
			gp.End = la.QuyB
		} else {
			gp.Begin = la.QuyE
			gp.End = la.QuyE + la.RefLen - la.RefE - len(AdpaterSeq)
		}
		if IsCrossGapRegArr(gapRegArr, gp) {
			extLa.RefE = la.RefLen - len(AdpaterSeq)
		}
	}
...
func SetLAFlag(laArr []LA, eID uint32) {
	for i, _ := range laArr {
...
func paraFindLongMappingPath(lac chan []LA, wc chan []uint32, edgesArr []DBGEdge, nodesArr []DBGNode, RefIDMapArr []uint32, consistenceA [][2][]uint32) {
	for {
		laArr := <-lac
		if len(laArr) == 0 {
			var guardPath []uint32
		...
		for {
			...
			var extArr [2][]uint32
			// if maxLa.RefB < Min(len(edge.Utg.Ks)/FLANK_ALLOW_FACTOR, MAX_FLANK_ALLOW_LEN)+len(AdpaterSeq) {
			flankLen := Kmerlen/REF_OVL_FACTOR + len(AdpaterSeq)
			for i := 0; i < 2; i++ {
				...
				if i == 0 {
					...
					if la.Minus {
						QuyPos = la.QuyE
						QuyDirection = FORWARD
					} else {
						QuyPos = la.QuyB
						QuyDirection = BACKWARD
					}
				} else {
					nID = edgesArr[eID].EndNID
					if la.Minus {
				...
				for nID > 0 {
					...
					if len(nlaArr) == 0 {
						...
						if SumLen(extArr[i], edgesArr) > MAX_INSERT_SIZE {
							...
							for j := 0; j < 2; j++ {
								...
								if IsSubPath(extArr[i], GetReverseuint32Arr(subp)) {
									neID = id
									idx = j
									num++
								}
					...
					// if IsInuint32Arr(extArr[i][:len(extArr[i])-1], eID) {
					// 	break
					// }
					// refresh next info
		...
		// flankLen := Min(len(nedge.Utg.Ks)/FLANK_ALLOW_FACTOR, MAX_FLANK_ALLOW_LEN) + len(AdpaterSeq)
...
func GetOrderID(edgesArr []DBGEdge) (ReadIDMapArr []uint32) {
	for _, e := range edgesArr {
		if e.ID > 0 {
...
func WriteLongPathToDBG(wc chan []uint32, edgesArr []DBGEdge, numCPU int) [][]Path {
	LongPathMatArr := make([][]Path, len(edgesArr))
	terminalNum := 0
	totalPathNum, totalPathLen := 0, 0
	for {
		path := <-wc
		...
		if len(path) > 3 {
			...
			if path[len(path)-1] == uint32(0) {
				path = path[:len(path)-1]
			}
		}
		if len(path) > 3 {
			//LongPathMatArr[path[0]] = InsertPathToEdge(LongPathMatArr[path[0]], path[1:], 1)
			path = Reverseuint32Arr(path)
			//LongPathMatArr[path[0]] = InsertPathToEdge(LongPathMatArr[path[0]], path[1:], 1)
...
func checkDBG(edgesArr []DBGEdge, nodesArr []DBGNode) {
...
/*
	...
	nodesArr := NodesArrReader(spNodesfn)
	// Restore edges info
	edgesStatfn := prefix + ".edges.stat"
	edgesSize := EdgesStatReader(edgesStatfn)
	edgesArr := make([]DBGEdge, edgesSize)
	edgesfn := prefix + ".edges.ShortPath.fq"
	LoadEdgesfqFromFn(edgesfn, edgesArr)
...
func GetEIDLenArr(edgesArr []DBGEdge, minLen int) (idLenArr []IDLen) {
...
func GetSameDirectionPath(pathArr []Path, eID uint32) (sdPathArr []Path) {
...
func AllowZeroDeepEqual(arr1, arr2 []uint32) bool {
	if arr1[0] != arr2[0] {
		return false
	}
	for i, _ := range arr1 {
...
func GetOverlapArr(arr []uint32, pathArr []Path) (overlapMat []Path) {
	for _, p := range pathArr {
...
/*func GetTotalFreq(pathArr []Path, idx int, eID uint32) (freqSum int) {
...
func CheckSignificantDiff(scoreArr []int, eIDArr []uint32) (eID uint32, ok bool) {
...
func GetMinCutPathArr(pathArr []Path, minPathFreq int) (consisPathArr []uint32) {
	for i := 0; ; i++ {
		var eIDArr []uint32
		var freqArr []int
		for _, p := range pathArr {
			if len(p.IDArr) > i {
				if idx := GetIndexuint32Arr(eIDArr, p.IDArr[i]); idx >= 0 {
					freqArr[idx] += p.Freq
				} else if p.IDArr[i] != 0 {
...
/*func GetMinCutPathArr(pathArr []Path, minPathFreq int) (consisPathArr []Path) {
	var freqArr [][]int
	for _, p := range pathArr {
		var added bool
...
func GetIndexuint32Arr(arr []uint32, ele uint32) int {
	idx := -1
	for i, id := range arr {
		if id == ele {
			idx = i
...
func GetIndexuint32ArrDirection(arr []uint32, ele uint32, direction uint8) int {
	idx := -1
	if direction == FORWARD {
		for i, id := range arr {
			if id == ele {
...
func GetCrossPathMat(neighbourEIDArr []uint32, LongPathMatArr [][]Path, eID uint32) (crossPathA []Path) {

	for _, id := range neighbourEIDArr {
		for _, p := range LongPathMatArr[id] {
			idx := GetIndexuint32Arr(p.IDArr, eID)
			if idx >= 0 {
...
func GetFlankPath(crossPathA []Path, eID, neID uint32) (fpArr []Path) {
		...
		if idx >= 0 {
			...
			if idx > 0 {
				if p.IDArr[idx-1] == neID {
					np.IDArr = GetReverseuint32Arr(p.IDArr[:idx])
					np.Freq = p.Freq
...
/*func ParseUniquePath(edgesArr []DBGEdge, nodesArr []DBGNode, LongPathMatArr [][]Path) (mergeMat [][]uint32) {
	...
	for _, IDLen := range IDLenArr {
		...
		catArr := GetReverseuint32Arr(pathArr[1])
		catArr = append(catArr, edge.ID)
					...
					for z := y; z <= x+1; z++ {
						for w := 0; w < len(pileArr[z]); w++ {
							if len(pileArr[z][w].IDArr) > len(pathArr[j])-z {
								...
								if p.IDArr[0] > 0 {
									//pa = InsertPathToEdge(pa, p.IDArr, p.Freq)
								}
							}
						}
						if len(pa) == 0 {
...
/*func IsBubbleEdge(e *DBGEdge, nodesArr []DBGNode, edgesArr []DBGEdge) (eIDArr []uint32, b bool) {
	...
	eIDArr = InterSecuint32Arr(arr1, arr2)
...
/*func IsBubble(eID1, eID2 uint32, edgesArr []DBGEdge) (b bool) {
...
func StaticsLongPath(LongPathMatArr [][]Path, edgesArr []DBGEdge, nodesArr []DBGNode) {
...
func StaticsMergeMat(matArr [][]uint32, edgesArr []DBGEdge) {
