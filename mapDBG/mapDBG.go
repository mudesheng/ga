...
func FindMEKArr(NoContainMKArr []MapEdgeKmers, eIDArr []constructdbg.DBG_MAX_INT, qPos int32) (proArr []MapEdgeKmers) {
	for _, eID := range eIDArr {
		var arr []MapEdgeKmers
		for _, m := range NoContainMKArr {
			if constructdbg.DBG_MAX_INT(m.EID) == eID {
				arr = append(arr, m)
			}
		}
		if len(arr) > 1 {
...
func MatchSeq(readSeq []byte, mk MapEdgeKmers, mks []MergeKmerInfo, edgeSeq []byte, seed uint32, direction uint8) (matchNum int32) {
	...
	if mk.Strand == 1 {
		...
		copy(tmp, edgeSeq)
		constructdbg.ReverseCompByteArr(tmp)
		cSeq = tmp
	}
...
func ExtendSeedEdge(seedEdgeIndex int, NoContainMKArr []MapEdgeKmers, edgesArr []constructdbg.DBGEdge, nodesArr []constructdbg.DBGNode, mks []MergeKmerInfo, seed int, ReadLen int32, readSeq []byte) (readPath []constructdbg.DBG_MAX_INT) {
	// extend left flank
	{
		smk := NoContainMKArr[seedEdgeIndex]
		readPath = append(readPath, constructdbg.DBG_MAX_INT(smk.EID))
		...
		for nID > 0 && qPos > 0 {
			...
			if len(proArr) == 0 {
				...
				readPath = append(readPath, constructdbg.DBG_MAX_INT(proArr[0].EID))
				qPos = proArr[0].Start
				...
				if proArr[1].End == proArr[1].End {
					...
					if matchBase[1] > matchBase[0] {
						readPath = append(readPath, constructdbg.DBG_MAX_INT(proArr[1].EID))
						qPos = proArr[1].Start
	...
	readPath = constructdbg.ReverseDBG_MAX_INTArr(readPath)
...
func paraFoundPath(rc chan []MergeKmerInfo, wc chan []constructdbg.DBG_MAX_INT, edgesArr []constructdbg.DBGEdge, nodesArr []constructdbg.DBGNode, seed int32, seqSlice []Seq) {
...
func WriteReadPath(wc chan []constructdbg.DBG_MAX_INT, numCPU int) {
	terminalNum := 0
	for {
		rp := <-wc
		if len(rp) == 0 {
...
func FindPath(mkiArr []MergeKmerInfo, numCPU, seed int, edgesArr []constructdbg.DBGEdge, nodesArr []constructdbg.DBGNode, seqSlice []Seq) {
	...
	wc := make(chan []constructdbg.DBG_MAX_INT, numCPU)
